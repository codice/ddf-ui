...
function translateFilterToBasicMap(filter) {
    ...
    if (propertyValueMap.datatype) {
        var validTypes_1 = StartupDataStore.MetacardDefinitions.getEnum(getMatchTypeAttribute());
        ...
        propertyValueMap.anyType.properties = propertyValueMap.datatype
            ...
            .filter(function (val) { return validTypes_1.includes(val); }); // filter out any datatypes that aren't in the options list. These can be added here from datatypeQueryMapping custom configuration
    }
    return {
...
var constructFilterFromBasicFilter = function (_a) {
    ...
    if (basicFilter.anyType.on && basicFilter.anyType.properties.length > 0) {
        var typeFilters = basicFilter.anyType.properties
        ...
        var datatypeQueryMapping = (_b = StartupDataStore.Configuration.getExtra()) === null || _b === void 0 ? void 0 : _b.datatypeQueryMapping;
        var datatypes = datatypeQueryMapping === null || datatypeQueryMapping === void 0 ? void 0 : datatypeQueryMapping.filter(function (m) {
            return basicFilter.anyType.properties.includes(m.datatype);
        });
        // In addition to the default datatype filter logic, add the custom datatype Query mapping from the configuration
        if ((datatypes === null || datatypes === void 0 ? void 0 : datatypes.length) > 0) {
            typeFilters = typeFilters.concat(datatypes.flatMap(function (datatype) {
                return datatype.attributes.flatMap(function (attribute) {
                    return attribute.values.flatMap(function (value) {
                        return new FilterClass({
