...
var BooleanSearchBar = function (_a) {
    var value = _a.value, onChange = _a.onChange, _b = _a.property, property = _b === void 0 ? 'anyText' : _b, _c = _a.placeholder, placeholder = _c === void 0 ? "Search ".concat(properties.customBranding, " ").concat(properties.product) : _c, disableClearable = _a.disableClearable, props = __rest(_a, ["value", "onChange", "property", "placeholder", "disableClearable"]);
    var _d = __read(React.useState(false), 2), isOpen = _d[0], setIsOpen = _d[1];
    ...
    var _g = __read(React.useState(''), 2), suggestion = _g[0], setSuggestion = _g[1];
    var _h = __read(React.useState(getRandomId()), 1), id = _h[0];
    var _j = __read(React.useState(0), 2), cursorLocation = _j[0], setCursorLocation = _j[1];
    var _k = __read(React.useState([]), 2), tokens = _k[0], setTokens = _k[1];
    var inputRef = React.useRef();
    var optionToValue = function (option) { return option.token; };
    var _l = __read(useState([]), 2), options = _l[0], setOptions = _l[1];
        ...
        if (searchVal && isValidBeginningToken(value.text)) {
            fetchCql({
                ...
                callback: function (_a) {
                    var _b = _a.cql, cql = _b === void 0 ? '' : _b, message = _a.message;
                    onChange(__assign(__assign({}, value), { cql: cql, error: Boolean(message) }));
                    setLoading(false);
                },
    ...
    React.useEffect(function () {
        var _a, _b;
        if (value.text) {
            var replaceIndex = value.text.indexOf('?');
            if (replaceIndex > -1) {
                // Make the selection around "?"
                (_a = inputRef === null || inputRef === void 0 ? void 0 : inputRef.current) === null || _a === void 0 ? void 0 : _a.setSelectionRange(replaceIndex, replaceIndex + 1);
            }
            if (suggestion === 'AND' || suggestion === 'OR') {
                (_b = inputRef === null || inputRef === void 0 ? void 0 : inputRef.current) === null || _b === void 0 ? void 0 : _b.setSelectionRange(cursorLocation, cursorLocation);
                setSuggestion('');
            }
        }
    }, [value.text]);
        ...
        React.createElement(Autocomplete, __assign({ filterOptions: function (optionsToFilter) { return optionsToFilter; }, onOpen: function () {
                ...
                return (React.createElement(TextField, __assign({ "data-id": "search-input" }, params, { onKeyUp: function (e) {
                        if (e.key === 'Enter') {
                            ...
                            !disableClearable && !!value.text && (React.createElement(IconButton, { onClick: handleTextClear, style: { padding: '2px' } },
                                React.createElement(ClearIcon, { fontSize: "small" }))),
                            React.createElement(IconButton, { onClick: function (e) {
                                    setIsOpen(false);
                                    handleSubmit(e);
                                }, disabled: value.error, style: { padding: '2px' } },
