...
export default function (insertionElement, _selectionInterface, _notificationEl, _componentElement, mapModel) {
    var overlays = {};
    var shapes = [];
    ...
    var exposedMethods = _.extend({}, Map, {
        ...
        onDoubleClick: function () {
            $(map.getTargetElement()).on('dblclick', function (e) {
                var boundingRect = map.getTargetElement().getBoundingClientRect();
                var id = determineLocationIdFromPosition([e.clientX - boundingRect.left, e.clientY - boundingRect.top], map);
                if (id) {
                    ;
