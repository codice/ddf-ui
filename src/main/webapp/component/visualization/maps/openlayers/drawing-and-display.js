...
var modelToPolygon = function (model) {
    ...
    var buffer = model.get('polygonBufferWidth');
    var bufferUnit = model.get('polygonBufferUnits');
    return makeGeometry(Common.generateUUID(), Turf.polygon(polygon).geometry, DRAWING_COLOR, 'Polygon', buffer ? parseInt(buffer) : undefined, bufferUnit || undefined);
};
var modelToLine = function (model) {
    ...
    var buffer = model.get('lineWidth');
    var bufferUnit = model.get('lineUnits');
    return makeLineGeo(Common.generateUUID(), coords, buffer ? parseInt(buffer) : 0, bufferUnit || 'meters');
};
var modelToPointRadius = function (model) {
    ...
    return makePointRadiusGeo(Common.generateUUID(), lat, lon, radius ? parseInt(radius) : 1, radiusUnits || 'meters');
};
...
var drawingLocation = makeEmptyGeometry(Common.generateUUID(), DEFAULT_SHAPE);
