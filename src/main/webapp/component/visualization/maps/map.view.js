...
var getLocation = function (model, translation) {
    ...
    switch (locationType) {
        case 'bbox':
            var bbox = _.pick(model.attributes, 'mapNorth', 'mapSouth', 'mapEast', 'mapWest');
            if (translation) {
                var _d = __read(translateCoordinates([bbox.mapEast, bbox.mapNorth], translation), 2), east = _d[0], north = _d[1];
                var _e = __read(translateCoordinates([bbox.mapWest, bbox.mapSouth], translation), 2), west = _e[0], south = _e[1];
                bbox.mapNorth = north;
                bbox.mapEast = east;
                bbox.mapSouth = south;
                bbox.mapWest = west;
        ...
        case 'circle':
            ...
            if (translation) {
                var _f = __read(translateCoordinates([point.lon, point.lat], translation), 2), lon = _f[0], lat = _f[1];
                point.lon = lon;
                point.lat = lat;
            }
            return point;
            ...
            if (translation) {
                try {
                    for (var line_1 = __values(line), line_1_1 = line_1.next(); !line_1_1.done; line_1_1 = line_1.next()) {
                        ...
                        var _g = __read(translateCoordinates(coord, translation), 2), lon = _g[0], lat = _g[1];
                        coord[0] = lon;
                        coord[1] = lat;
                    }
                }
        ...
        case 'poly':
            ...
            if (translation) {
                ...
                try {
                    for (var multiPolygon_1 = __values(multiPolygon), multiPolygon_1_1 = multiPolygon_1.next(); !multiPolygon_1_1.done; multiPolygon_1_1 = multiPolygon_1.next()) {
                        var ring = multiPolygon_1_1.value;
                        try {
                            for (var ring_1 = (e_3 = void 0, __values(ring)), ring_1_1 = ring_1.next(); !ring_1_1.done; ring_1_1 = ring_1.next()) {
                                var coord = ring_1_1.value;
                                var _h = __read(translateCoordinates(coord, translation), 2), lon = _h[0], lat = _h[1];
                                coord[0] = lon;
                                coord[1] = lat;
                            }
                        }
                            ...
                            try {
                                if (ring_1_1 && !ring_1_1.done && (_c = ring_1.return)) _c.call(ring_1);
                            }
                    ...
                    try {
                        if (multiPolygon_1_1 && !multiPolygon_1_1.done && (_b = multiPolygon_1.return)) _b.call(multiPolygon_1);
                    }
...
var translateCoordinates = function (coord, // [lon, lat]
translation) {
    var newCoords = __spreadArray([], __read(coord), false);
