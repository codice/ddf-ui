...
import { CQLStandardFilterBuilderClass, deserialize, FilterBuilderClass, FilterClass, isFilterBuilderClass, serialize, shouldBeFilterBuilderClass, } from '../component/filter-builder/filter.structure';
        ...
        case 'SPATIAL': // working
            ...
            switch (tok.text) {
                case 'INTERSECTS': {
                    ...
                    var propertyToken_1 = postfix.pop();
                    if (valueToken.text.startsWith('LINESTRING')) {
                        ...
                        return new FilterBuilderClass({
                            type: 'OR',
                            filters: CQLUtils.arrayFromPolygonWkt(valueToken.text).map(function (poly) {
                                return new FilterClass({
                                    type: 'GEOMETRY',
                                    value: {
                                        mode: 'poly',
                ...
                case 'DWITHIN': {
                    ...
                    var valueToken = postfix.pop();
                    var propertyToken_2 = postfix.pop();
                    if (valueToken.text.startsWith('LINESTRING')) {
                        return new FilterClass({
                            type: 'GEOMETRY',
                            value: {
...
function uncollapseNOTs(_a) {
    ...
    if (isFilterBuilderClass(cqlAst)) {
        ...
        if (cqlAst.negated) {
            var clonedFieldFilter = JSON.parse(JSON.stringify(cqlAst));
            return new CQLStandardFilterBuilderClass({
                type: 'NOT',
                filters: [
                    new CQLStandardFilterBuilderClass({
...
export default {
    ...
    write: function (filter) {
        try {
            ...
            var standardCqlAst = uncollapseNOTs({
                cqlAst: filter
            });
