...
function assertMultiPolygon(actual, expected) {
    expect(actual.length).equals(expected.length);
    actual.forEach(function (polygon, i) {
...
describe('CQL Utils', function () {
    ...
    describe('generates filters', function () {
        ...
        it('generates filter with anyGeo property and POINTRADIUS type', function () {
            var filter = CQLUtils.generateFilter('some type', 'anyGeo', { type: 'POINTRADIUS', lon: 2, lat: 3, radius: 10 }, mockMetacardDefinitions);
            expect(filter.type).equals('DWITHIN');
            expect(filter.property).equals('anyGeo');
        ...
        it('generates filter with anyText property', function () {
            var filter = CQLUtils.generateFilter('some type', 'anyText', 'some value', mockMetacardDefinitions);
        ...
        it('generates DURING filter with temporal property', function () {
            var filter = CQLUtils.generateFilter('DURING', 'created', '2018-11-01T19:00:00.000Z/2018-11-30T19:00:00.000Z', mockMetacardDefinitions);
...
//# sourceMappingURL=CQLUtils.spec.js.map