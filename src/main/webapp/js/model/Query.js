...
export default Backbone.AssociatedModel.extend({
    ...
    initialize: function (attributes) {
        ...
        this.listenTo(this, 'change:cql change:filterTree change:sources change:sorts change:spellcheck change:phonetics change:count', function () {
    ...
    getSelectedSources: function () {
        ...
        if (selectedSources.includes('remote')) {
            sourceArray = sourceArray
                .concat(_.pluck(Sources.toJSON(), 'id').filter(function (src) { return !Sources.getHarvested().includes(src); }))
    ...
    initializeResult: function (options) {
        ...
        var harvestedSources = Sources.getHarvested();
    ...
    startSearch: function (options, done) {
        ...
        this.currentSearches = searchesToRun.map(function (search) {
            ...
            return result.fetch(__assign(__assign({}, CommonAjaxSettings), { customErrorHandling: true, data: JSON.stringify(search), remove: false, dataType: 'json', contentType: 'application/json', method: 'POST', processData: false, timeout: properties.timeout, success: function (_model, response, options) {
