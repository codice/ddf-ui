...
export var getLabel = function (value) {
    var label = metacardDefinitions.getLabel(value);
    if (label === 'RELEVANCE') {
...
export var getSortAttributeOptions = function (currentSelections) {
    ...
    var attributes = metacardDefinitions.sortedMetacardTypes;
    var options = attributes
        .filter(function (type) { return !properties.isHidden(type.id); })
        .filter(function (type) { return !metacardDefinitions.isHiddenTypeExceptThumbnail(type.id); })
