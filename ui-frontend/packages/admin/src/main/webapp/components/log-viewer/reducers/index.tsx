...
const filter = (
  state = { level: 'ALL' },
  { type, filter }: { type: any; filter: any }
) => {
  switch (type) {
    case 'FILTER_LOGS':
      return { ...state, ...filter }
...
const logs = (state = [], { type, entries }: { type: any; entries: any }) => {
  switch (type) {
    case 'APPEND_LOGS':
...
const displaySize = (state = INITIAL_DISPLAY_SIZE, { type }: { type: any }) => {
  switch (type) {
...
const isPolling = (state = false, { type }: { type: any }) => {
  switch (type) {
    case 'TOGGLE_POLLING':
      return !state
    case 'SHOW_ERROR':
...
const isFetching = (state = false, { type }: { type: any }) => {
  switch (type) {
    case 'BEGIN_FETCHING':
      return true
    case 'END_FETCHING':
