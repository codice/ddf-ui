...
class MaskedTextField extends React.Component {
    ...
    render() {
        ...
        <Group>
          {label != null ? (<div className="p-2 flex-shrink-0 flex-grow-0" style={{
                    minWidth: '120px',
                }}>
              {label}
              &nbsp;
            </div>) : null}
          <MaskedInput className="inline-block w-full whitespace-no-wrap flex-shrink overflow-hidden" value={value} keepCharPositions onChange={(e: any) => {
                (this.props as any).onChange(e.target.value);
            }} pipe={(value: any) => this.padEndWithZeros(value)} render={(setRef: any, { defaultValue, ...props }: any) => {
                return (<TextField size="small" fullWidth variant="outlined" id={label} inputRef={(ref) => {
                        setRef(ref);
                        this.ref = ref;
                    }} value={defaultValue || ''} {...props}/>);
            }} {...args}/>
          {addon != null ? (<label className="p-2 flex-shrink-0 flex-grow-0">{addon}</label>) : null}
        </Group>
      </div>);
    }
}
