...
type ResizableGridType = React.ComponentType<ResizableProps & {
    component: any;
    item: any;
}>;
const ResizableGrid = Grid as ResizableGridType;
export const [useResizableGridContext, UseResizableGridContextProvider,] = createCtx<useResizableGridType>({
    closed: false,
});
...
export const useResizableGrid = ({ startingLength, collapsedLength, autoCollapseLength, }: {
...
export const SplitPane = ({ firstStyle, secondStyle, variant, children, collapsedLength = DEFAULT_COLLAPSED_LENGTH, autoCollapseLength = DEFAULT_AUTO_COLLAPSE_LENGTH, startingLength = DEFAULT_STARTING_LENGTH, }: SplitPaneProps) => {
    const { length, closed, setClosed, setLength, lastLength, setLastLength, dragging, setDragging, } = useResizableGrid({ collapsedLength, startingLength, autoCollapseLength });
    const [First, Second] = children;
    return (<UseResizableGridContextProvider value={{
            length,
            closed,
            setClosed,
            setLength,
      ...
      <Grid container wrap="nowrap" direction={(() => {
            switch (variant) {
                case 'horizontal':
                    return 'row';
                case 'vertical':
        ...
        <CustomResizableGrid component={Resizable} item size={(() => {
            switch (variant) {
                case 'horizontal':
                    return {
                        width: length,
        ...
        })()} minWidth={collapsedLength} enable={(() => {
            switch (variant) {
                case 'horizontal':
                    return {
                        top: false,
        ...
        // @ts-expect-error ts-migrate(6133) FIXME: 'e' is declared but its value is never read.
            ...
            switch (variant) {
                case 'horizontal':
                    setLength((e as any).clientX - (e.target as any).parentElement.getBoundingClientRect().x);
                    break;
                case 'vertical':
                    setLength((e as any).clientY - (e.target as any).parentElement.getBoundingClientRect().y);
                    break;
            }
        }} className="z-10 pr-2">
          {First}
