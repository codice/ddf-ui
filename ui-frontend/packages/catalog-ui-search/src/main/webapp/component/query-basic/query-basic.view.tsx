...
import QueryTimeReactView, { BasicFilterClass, } from '../query-time/query-time.view';
const METADATA_CONTENT_TYPE = 'metadata-content-type';
import TextField from '@material-ui/core/TextField';
import FormControlLabel from '@material-ui/core/FormControlLabel';
import Checkbox from '@material-ui/core/Checkbox';
import { BooleanTextType, FilterBuilderClass, FilterClass, } from '../filter-builder/filter.structure';
...
function getMatchTypeAttribute() {
    return metacardDefinitions.metacardTypes[(properties as any).basicSearchMatchType]
...
function getAllValidValuesForMatchTypeAttribute(): {
    ...
    return metacardDefinitions.enums[matchTypeAttr]
        ? (metacardDefinitions.enums[matchTypeAttr] as Array<string>).reduce((blob, value: any) => {
            blob[value] = {
                label: value,
                value,
...
const determinePropertiesToApplyTo = ({ value, }: {
...
function isTypeLimiter(filter: any) {
    ...
    const metadataContentTypeSupported = Boolean(metacardDefinitions.metacardTypes[METADATA_CONTENT_TYPE]);
...
function isAnyDate(filter: any) {
    if (!filter.filters) {
        return (metacardDefinitions.metacardTypes[stripQuotes(filter.property)].type ===
            'DATE');
    }
    ...
    if (typesFound.length > 1 || valuesFound.length > 1) {
        ...
        const attributes = filter.filters.map((subfilter: any) => subfilter.property);
        return (metacardDefinitions.metacardTypes[stripQuotes(attributes[0])].type ===
            'DATE');
    }
}
function handleAnyDateFilter(propertyValueMap: any, filter: any) {
    propertyValueMap['anyDate'] = propertyValueMap['anyDate'] || [];
    let existingFilter = propertyValueMap['anyDate'].filter((anyDateFilter: any) => anyDateFilter.type ===
        (filter.filters ? filter.filters[0].type : filter.type))[0];
    if (!existingFilter) {
        existingFilter = {
            property: [],
        };
...
export function downgradeFilterTreeToBasic(filter: FilterBuilderClass): FilterBuilderClass {
        ...
        filter.filters.forEach((filter: any) => {
            if (!filter.filters && isAnyDate(filter)) {
                ...
                propertyValueMap[CQLUtils.getProperty(filter)] =
                    propertyValueMap[CQLUtils.getProperty(filter)] || [];
                if (propertyValueMap[CQLUtils.getProperty(filter)].filter((existingFilter: any) => existingFilter.type === filter.type).length === 0) {
                ...
                propertyValueMap[CQLUtils.getProperty(filter.filters[0])] =
                    propertyValueMap[CQLUtils.getProperty(filter.filters[0])] || [];
                filter.filters
                    .filter((subfilter: FilterClass) => CQLUtils.getProperty(subfilter) ===
...
const constructFilterFromBasicFilter = ({ basicFilter, }: {
    ...
    if (basicFilter.anyType.on && basicFilter.anyType.properties.length > 0) {
        filters.push(new FilterBuilderClass({
            ...
            filters: basicFilter.anyType.properties
                ...
                .concat(basicFilter.anyType.properties.map((property) => {
                return new FilterClass({
...
const useBasicFilterFromModel = ({ model }: QueryBasicProps) => {
    const [basicFilter, setBasicFilter] = React.useState(translateFilterToBasicMap(getFilterTree(model)).propertyValueMap);
...
const QueryBasic = ({ model }: QueryBasicProps) => {
    ...
    const [typeAttributes] = React.useState(getAllValidValuesForMatchTypeAttribute());
    /**
    ...
    return (<>
      <div className="editor-properties px-2 py-3">
        <div className="">
          ...
          <BooleanSearchBar value={anyTextValue} key={model.id} onChange={({ text, cql, error }) => {
        ...
        <div className="pt-2">
          <QueryTimeReactView value={basicFilter.anyDate[0]} onChange={(newValue) => {
            basicFilter.anyDate[0] = newValue;
            model.set('filterTree', constructFilterFromBasicFilter({ basicFilter }));
        }}/>
        </div>
        <div className="pt-2">
          <FormControlLabel labelPlacement="end" control={<Checkbox color="default" checked={Boolean(basicFilter.anyGeo[0])} onChange={(e) => {
                if (!e.target.checked) {
          ...
          {basicFilter.anyGeo[0] ? (<Grid container alignItems="stretch" direction="row" wrap="nowrap" className="pt-2">
              <Grid item>
                <Swath className="w-1 h-full"/>
              </Grid>
              <Grid item className="w-full pl-2">
                <FilterInput filter={new FilterClass({
        ...
        <div className="pt-2">
          <FormControlLabel labelPlacement="end" control={<Checkbox color="default" checked={basicFilter.anyType.on} onChange={(e) => {
                basicFilter.anyType.on = e.target.checked;
                model.set('filterTree', constructFilterFromBasicFilter({ basicFilter }));
            }}/>} label="Types"/>
          {basicFilter.anyType.on ? (<Grid container alignItems="stretch" direction="row" wrap="nowrap" className="pt-2">
              <Grid item>
                <Swath className="w-1 h-full"/>
              </Grid>
              <Grid item className="w-full pl-2">
                <Autocomplete fullWidth multiple options={Object.values(typeAttributes)} disableCloseOnSelect getOptionLabel={(option) => option.label} getOptionSelected={(option, value) => option.value === value.value} onChange={(_e, newValue) => {
                basicFilter.anyType.properties = newValue.map((val) => val.value);
                model.set('filterTree', constructFilterFromBasicFilter({ basicFilter }));
            }} size="small" renderOption={({ label, value }) => {
                return (<>
                        <div className={`pr-2 icon ${typeAttributes[value].class}`}/>
                        {label}
                      </>);
            }} renderTags={(tagValue, getTagProps) => tagValue.map((option, index) => (<Chip variant="outlined" color="default" label={<>
                            <div className={`pr-2 icon ${typeAttributes[option.value].class}`}/>
                            {option.label}
                          </>} {...getTagProps({ index })}/>))} value={determinePropertiesToApplyTo({
