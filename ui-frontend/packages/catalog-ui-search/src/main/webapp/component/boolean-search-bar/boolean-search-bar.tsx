...
import { fetchCql, fetchSuggestions, getRandomId, Option, } from './boolean-search-utils';
 ...
 * back down.
 */
const BooleanSearchBar = ({ value, onChange, property = 'anyText', placeholder = `Search ${(properties as any).customBranding} ${(properties as any).product}`, disableClearable, ...props }: Props) => {
    ...
    const filterOptions = React.useCallback((optionsToFilter) => {
        ...
        if (lastToken === '') {
            ...
            const reconstructedOptions = optionsToFilter.filter((o: any) => flatFilteredOptions.includes(o.token));
    ...
    return (<FormControl fullWidth {...props.FormControlProps}>
      <Autocomplete filterOptions={(optionsToFilter) => filterOptions(optionsToFilter).sort((o1: any) => o1.type === 'mandatory' ? -1 : 1)} options={getLogicalOperators(options)} includeInputInList={true} onChange={(_e: any, suggestion: any) => {
            if (suggestion &&
                suggestion.token &&
                ...
                if (tokenToRemove !== '' && cursor && cursor < value.text.length) {
                    ...
                    const preText = value.text.slice(0, (tokenToRemove.length + postText.length) * -1);
                    onChange({
                        ...value,
        ...
        }} inputValue={value.text} getOptionLabel={getOptionLabel} multiple={false} disableClearable disableCloseOnSelect freeSolo id={id} renderOption={(option) => (<Typography noWrap>{optionToValue(option)}</Typography>)} renderInput={(params) => (<TextField data-id="search-input" {...params} onKeyDown={(e) => {
                if (e.keyCode === 13) {
                    handleSubmit();
                }
            }} placeholder={placeholder} inputRef={inputRef} size={'small'} variant="outlined" defaultValue={'*'} onChange={handleTextChange} value={value.text} autoFocus helperText={value.error ? <>{errorMessage}</> : ''} InputProps={{
                ...params.InputProps,
                startAdornment: (<>
                  {loading ? (<CircularProgress size={20} style={{ marginRight: 13, marginLeft: 2 }}/>) : (<ValidationIndicator helperMessage={value.error ? errorMessage : 'Valid'} error={value.error}/>)}
                </>),
                endAdornment: (<>
                  {!disableClearable && !!value.text && (<IconButton onClick={handleTextClear} style={{ padding: '2px' }}>
                      <ClearIcon fontSize="small"/>
                    </IconButton>)}
                  {props.onSubmit && (<IconButton onClick={handleSubmit} disabled={value.error} style={{ padding: '2px' }}>
                      <SearchIcon fontSize="small"/>
                    </IconButton>)}
                </>),
                ...props.InputProps,
            }} {...props.TextFieldProps}/>)} {...props.AutocompleteProps}/>
    </FormControl>);
};
