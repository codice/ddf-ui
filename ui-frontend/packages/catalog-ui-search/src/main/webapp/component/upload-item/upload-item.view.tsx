...
export const UploadItemViewReact = ({ model }: UploadItemViewReactType) => {
    ...
    useListenTo(model, 'change:percentage change:sending change:success change:error change:validating change:issues', () => {
    ...
    return (<div className={`flex flex-row items-center flex-no-wrap w-full p-4 border-gray-600 border border-opacity-25`} onClick={() => {
      ...
      <div className="w-full flex-shrink">
        ...
        {!hasSuccess && !hasError && isSending ? (<LinearProgress className="h-2 w-full" value={modelJson.percentage} variant="determinate"/>) : null}

        {hasSuccess ? (<div className="info-success text-center">
            <div className="success-message">
              {hasIssues ? (<span>Uploaded, but quality issues were found </span>) : (<></>)}
              {isValidating ? (<span className="success-validate fa fa-refresh fa-spin is-critical-animation"></span>) : (<></>)}
              {hasIssues ? <span className="message-text"></span> : <></>}
            </div>
          </div>) : null}
        {hasError ? (<div className="info-error text-center">
            <div className="error-message">{modelJson.message}</div>
          </div>) : null}
      </div>

      <div className="upload-actions flex-shrink-0">
        {!isSending ? (<Button className="" onClick={() => {
                setCancel(true);
            }}>
            Remove
          </Button>) : null}

        {hasSuccess ? (<Button className="" onClick={() => {
                (wreqr as any).vent.trigger('router:navigate', {
                    fragment: 'metacards/' + model.get('id'),
                    options: {
                        trigger: true,
