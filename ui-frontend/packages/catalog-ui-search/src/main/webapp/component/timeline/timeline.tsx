...
const Root = styled.div `
  ...
  .data {
    fill: ${({ theme: { theme } }) => theme === 'dark' ? lighten(0.7, 'black') : lighten(0.3, 'black')};
    fill-opacity: 0.7;
    :hover {
      stroke-width: 2px;
      stroke: ${({ theme }) => theme.primaryColor};
...
const generateTooltipMessage = (data: string[]) => {
    ...
    const otherResults = (<React.Fragment>
      <br />
 ...
 */
const hideElement = (element: d3.Selection<null, unknown, null, undefined>) => element.attr('style', 'display: none');
/**
 * Given a d3 selection, set the display to block.
 */
const showElement = (element: d3.Selection<null, unknown, null, undefined>) => element.attr('style', 'display: block');
/**
 * Domain is the minimum and maximum values that the scale contains.
 */
const getTimescaleFromWidth = (width: number, min: Moment = moment('1980-01-01:00:00.000z'), max: Moment = moment()): Timescale => {
...
export const Timeline = (props: TimelineProps) => {
    ...
    const [xAxis, setXAxis] = useState(() => d3.axisBottom(xScale).tickSize(AXIS_HEIGHT).tickFormat(multiFormat));
    ...
    useEffect(() => {
        ...
        const renderInitialXAxis = () => {
            ...
            svg
                .select('.axis--x')
                .attr('transform', `translate(0 ${height - (AXIS_MARGIN + AXIS_HEIGHT + heightOffset)})`)
                .call(xAxis);
        };
    ...
    useEffect(() => {
        ...
        if (props.data && selectedDateAttribute !== undefined) {
            ...
            buckets.forEach((b, i) => {
                ...
                const y = height - rectangleHeight - (AXIS_MARGIN + AXIS_HEIGHT + heightOffset);
    ...
    useEffect(() => {
        if (!isDragging &&
            ...
            if (selectionRange.length == 2) {
                ...
                const dataToSelect = _.flatMap(bucketsContainingRelevantData, (b) => b.items).filter((d) => dateWithinRange(d.attributes[selectedDateAttribute!], selectionRange));
    ...
    useEffect(() => {
        ...
        const getSelectionDrag = () => {
            ...
            return (d3
                ...
                const newLeftDate = moment.tz(xScale.invert(clickStart), props.timezone);
                ...
                if (props.mode !== 'single') {
                    ...
                    const initialDate = moment.tz(xScale.invert(clickStart), props.timezone);
                    let dragCurrent = clickStart + diff;
                    const dragDate = moment.tz(xScale.invert(dragCurrent), props.timezone);
                    if (diff > 0) {
    ...
    useEffect(() => {
         ...
         */
        const getEdgeDrag = (slider: 'LEFT' | 'RIGHT') => d3
            .drag()
            .on('start', () => {
            hideElement(d3.select(hoverLineRef.current));
            hideElement(d3.select(hoverLineTextRef.current));
    ...
    useEffect(() => {
        ...
        const getBrushDrag = () => d3
            ...
            const newRightDate = moment.tz(xScale.invert(newRight), props.timezone);
        ...
        if (leftMarkerRef.current &&
            ...
            if (props.mode === 'single' && selectionRange.length === 1) {
                ...
                leftMarker
                    ...
                    .attr('style', 'display: block');
                rightMarker
                    .attr('transform', `translate(${xScale(rightValue)}, ${markerHeight})`)
                    .attr('style', 'display: block');
                brushBar
                    .attr('transform', `translate(${xScale(leftValue)},${markerHeight})`)
    ...
    const renderCopyableDate = (date: Moment) => {
        const formattedDate = convertToDisplayable(date, props.timezone, props.format);
        return (<>
        <br />
    ...
    const renderContext = () => {
        const renderStartAndEnd = () => (<React.Fragment>
        <TimeText>
          <b>Start</b>
        ...
        if (props.mode === 'single') {
            if (!selectionRange[0]) {
                return (<Message>Click to select a time. Zoom with the scroll wheel.</Message>);
            }
            return (<TimeText>
          <b>Time</b>
          {selectionRange[0] && renderCopyableDate(selectionRange[0])}
            ...
            if (!selectionRange[0]) {
                return (<Message>Drag to select a range. Zoom with the scroll wheel.</Message>);
            }
    ...
    return (<Root ref={rootRef} style={{ height: '100%' }}>
      <div>
        <DateAttributeSelect visible={props.data && props.data!.length > 0} variant="outlined" onChange={(e: any) => setSelectedDateAttribute(e.target.value)} value={selectedDateAttribute}>
          {possibleDateAttributes.map((dateAttribute: string) => (<MenuItem value={dateAttribute}>
              {lookupAlias(dateAttribute)}
            </MenuItem>))}
        </DateAttributeSelect>
      </div>
      {tooltip && (<Tooltip message={tooltip.message} x={tooltip.x} y={tooltip.y}/>)}
      <svg ref={d3ContainerRef}>
        ...
        <HoverLineText x="0" y="0" style={{ display: 'none' }} ref={hoverLineTextRef}/>
        ...
        <g className="axis axis--x" id="axis">
          <rect width={width} height={AXIS_HEIGHT + AXIS_MARGIN} fillOpacity="0" fill="black"/>
        </g>
      </svg>
      <ContextRow>
        {renderContext()}
        <ButtonArea>
          ...
          {props.onDone && props.mode && (<TimelineButton color="primary" variant="contained" onClick={() => {
