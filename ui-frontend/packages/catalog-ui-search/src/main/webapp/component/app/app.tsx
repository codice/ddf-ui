...
import { Switch, Route, useLocation, useHistory, RouteProps, LinkProps, } from 'react-router-dom';
...
import UserView, { RoleDisplay, RoleIcon, SmallRoleDisplay, } from '../../react-component/user/user';
import UserSettings, { SettingsComponentType, } from '../../react-component/user-settings/user-settings';
...
const AsyncTasksComponent = () => {
            ...
            return task.subscribeTo({
                ...
                callback: () => {
                    ...
                    if (AsyncTasks.isRestoreTask(task)) {
                        addSnack(`Restore of ${task.lazyResult.plain.metacard.properties.title} complete.`, {
                            timeout: 5000,
                            closeable: true,
                            alertProps: {
                                action: (<Button component={Link} to={`/search/${task.lazyResult.plain.metacard.properties['metacard.deleted.id']}`}>
                      Go to
                    </Button>),
                            },
                        });
                    ...
                    if (AsyncTasks.isDeleteTask(task)) {
                        addSnack(`Delete of ${task.lazyResult.plain.metacard.properties.title} complete.`, {
                            undo: () => {
                                history.push({
    ...
    if (AsyncTasks.list.length > 0) {
        return (<div className={`${showBar ? 'translate-y-0' : 'translate-y-full'} absolute left-0 bottom-0 w-full bg-black bg-opacity-75 h-16 z-50 transition transform ease-in-out duration-500 hover:translate-y-0`}>
        <LinearProgress className="w-full absolute h-2 absolute left-0 top-0 -mt-2" variant="indeterminate"/>
...
const HelpButton = () => {
    ...
    return (<ExpandingButton component={(properties as any).helpUrl ? 'a' : Link} href={(properties as any).helpUrl} to={(properties as any).helpUrl
            ...
            }} target={(properties as any).helpUrl ? '_blank' : undefined} className={`group-hover:opacity-100 opacity-25  hover:opacity-100 focus-visible:opacity-100 transition-opacity`} Icon={HelpIcon} expandedLabel="Help" dataId={sidebarDataIdTag('help')} expanded={navOpen} focusVisibleClassName="focus-visible"/>);
};
const SettingsButton = () => {
    ...
    return (<>
      ...
      <ExpandingButton component={Link} to={{
            pathname: `${location.pathname}`,
            search: `${queryString.stringify({
                ...queryParams,
                'global-settings': 'Settings',
            })}`,
        }} className={`group-hover:opacity-100 opacity-25 relative hover:opacity-100 focus-visible:opacity-100 transition-opacity`} Icon={SettingsIcon} expandedLabel="Settings" dataId={sidebarDataIdTag('settings')} expanded={navOpen} focusVisibleClassName="focus-visible"/>
...
const NotificationsButton = () => {
    ...
    const { navOpen } = useNavContextProvider();
    return (<>
      <div className={hasUnseenNotifications ? 'animate-wiggle Mui-text-warning' : ''}>
        {/* @ts-expect-error ts-migrate(2322) FIXME: Type 'ForwardRefExoticComponent<LinkProps<UnknownF... Remove this comment to see the full error message */}
        <ExpandingButton component={Link} to={{
            pathname: `${location.pathname}`,
            search: `${queryString.stringify({
        ...
        }} className={`${!hasUnseenNotifications ? 'opacity-25' : ''} group-hover:opacity-100  relative hover:opacity-100 focus-visible:opacity-100 transition-opacity`} Icon={NotificationsIcon} expandedLabel="Notifications" dataId={sidebarDataIdTag('notifications')} expanded={navOpen} focusVisibleClassName="focus-visible" orientation="vertical"/>
      </div>
      <Drawer anchor="left" open={open} onClose={() => {
...
const UserButton = () => {
    ...
    return (<>
      ...
      <ExpandingButton component={Link} to={{
        ...
        }} className={`group-hover:opacity-100 opacity-25 relative hover:opacity-100 focus-visible:opacity-100 transition-opacity`} Icon={RoleIcon} expandedLabel={<div className="w-full">
            <div className="truncate w-full">{userInstance.getUserName()}</div>
            <div className="text-base truncate w-full">
              <RoleDisplay />
            </div>
          </div>} unexpandedLabel={<div className="w-full">
            <div className="w-full truncate">{userInstance.getUserName()}</div>
            <div className="text-xs truncate w-full">
              <SmallRoleDisplay />
            </div>
          </div>} dataId={sidebarDataIdTag('user-profile')} expanded={navOpen} focusVisibleClassName="focus-visible"/>
      <Drawer anchor="left" open={open} onClose={() => {
...
const SideBarRoutes = () => {
    ...
    return (<Grid item className="overflow-auto p-0 flex-shrink-0 scrollbars-min" style={{
            maxHeight: `calc(100% - ${7 * 4}rem)`, //
        }}>
      {RouteInformation.filter((routeInfo) => routeInfo.showInNav).map((routeInfo: RouteShownInNavType) => {
...
const SideBarToggleButton = () => {
    ...
    return (<>
      <Grid item className="w-full h-16 flex-shrink-0">
        {navOpen ? (<>
            <Grid container wrap="nowrap" alignItems="center" className="w-full h-full overflow-hidden">
              <Grid item className="pl-3 py-1 pr-1 w-full relative h-full">
                {(properties as any).topLeftLogoSrc ? (<img className="max-h-full max-w-full absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 top-1/2 p-4" src={handleBase64EncodedImages((properties as any).topLeftLogoSrc)}/>) : (<Grid container direction="column" className="pl-3" justify="center">
                    <Grid item>
                      <Typography>{(properties as any).customBranding}</Typography>
                    </Grid>
                    <Grid item>
              ...
              <Grid item className="ml-auto">
                <IconButton className="h-auto" onClick={() => {
                setNavOpen(false);
            }}>
                  <ChevronLeftIcon />
                </IconButton>
            ...
            {(properties as any).menuIconSrc ? (<>
                <img src={handleBase64EncodedImages((properties as any).menuIconSrc)} className="max-h-16 max-w-full"/>
              </>) : (<MenuIcon />)}
          </Button>)}
      </Grid>
    </>);
...
const SideBar = () => {
    const { navOpen } = useNavContextProvider();
    return (<Grid item className={`${navOpen ? 'w-64' : 'w-20'} transition-all duration-200 ease-in-out relative z-10 mr-2 flex-shrink-0 pb-2 pt-2 pl-2 group`} onMouseLeave={() => {
            scrollCurrentRouteIntoView();
        }}>
      <Paper elevation={Elevations.navbar} className="h-full">
        <Grid container direction="column" className="h-full w-full" wrap="nowrap">
          <SideBarToggleButton />
          <Divider />
          <SideBarRoutes />
          <Divider />
          <SideBarBackground />
          <Divider />
          <Grid item className="mt-auto overflow-hidden w-full flex-shrink-0 flex-grow-0">
            <HelpButton />
            <SettingsButton />
...
const Header = () => {
    return (<Grid item className="w-full">
      {(properties.ui as any).header ? (<Typography align="center" style={{
...
const Footer = () => {
    return (<Grid item className="w-full">
      {(properties.ui as any).footer ? (<Typography align="center" style={{
                background: (properties.ui as any).background,
                color: (properties.ui as any).color,
            }}>
...
const SideBarBackground = () => {
    return (<Grid item className="relative overflow-hidden flex-shrink-1 h-full min-w-full">
      {(properties as any).bottomLeftBackgroundSrc ? (<img className={`group-hover:opacity-100 opacity-50 duration-200 ease-in-out transition-all w-auto h-full absolute max-w-none m-auto min-h-80`} src={handleBase64EncodedImages((properties as any).bottomLeftBackgroundSrc)}/>) : null}
    </Grid>);
};
const RouteContents = () => {
    const { RouteInformation } = useTopLevelAppContext();
     ...
     */
    return (<Grid item className="w-full h-full relative z-0 flex-shrink-1 overflow-x-hidden" // do not remove this overflow hidden, see comment above for more
    >
      <Memo>
        <Switch>
          {RouteInformation.map((routeInfo: RouteNotShownInNavType) => {
            return (<Route key={routeInfo.routeProps.path
...
const useIndicateHasUnseenNotifications = () => {
    ...
    const [hasUnseenNotifications, setHasUnseenNotifications] = React.useState(notifications.hasUnseen() as boolean);
    React.useEffect(() => {
...
const useNavOpen = () => {
    let defaultOpen = localStorage.getItem('shell.drawer') === 'true' ? true : false;
...
const SessionTimeoutComponent = () => {
    ...
    return (<sessionTimeoutDialogState.MuiDialogComponents.Dialog {...sessionTimeoutDialogState.MuiDialogProps} disableEscapeKeyDown disableBackdropClick>
      <SessionTimeout />
    </sessionTimeoutDialogState.MuiDialogComponents.Dialog>);
};
const App = ({ RouteInformation, NotificationsComponent, SettingsComponents, }: AppPropsType) => {
    const { navOpen, setNavOpen } = useNavOpen();
    useFaviconBranding();
    useBackboneRouteReactRouterCompatibility();
    useScrollCurrentRouteIntoViewOnLocationChange();
    return (<TopLevelAppContext.Provider value={{ RouteInformation, NotificationsComponent, SettingsComponents }}>
      <NavContextProvider.Provider value={{ navOpen, setNavOpen }}>
        <div className="h-full w-full overflow-hidden Mui-bg-default">
          ...
          <AjaxErrorHandling />
          <WreqrSnacks />
          <Grid container alignItems="center" className="h-full w-full overflow-hidden" direction="column" wrap="nowrap">
            <Header />
            <Grid item className="w-full h-full relative overflow-hidden">
              <AsyncTasksComponent />
              <Grid container direction="row" wrap="nowrap" alignItems="stretch" className="w-full h-full">
                <SideBar />
                <RouteContents />
              </Grid>
            </Grid>
