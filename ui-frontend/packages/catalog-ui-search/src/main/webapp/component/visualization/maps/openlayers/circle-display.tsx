...
export function translateFromOpenlayersCoordinate(coord: any) {
    return ol.proj.transform([Number(coord[0]), Number(coord[1])], (properties as any).projection, 'EPSG:4326');
}
function translateToOpenlayersCoordinate(coord: any) {
    return ol.proj.transform([Number(coord[0]), Number(coord[1])], 'EPSG:4326', (properties as any).projection);
}
...
const modelToCircle = ({ model, map }: {
    ...
    const rectangle = new ol.geom.Circle(translateToOpenlayersCoordinate([model.get('lon'), model.get('lat')]), DistanceUtils.getDistanceInMeters(model.get('radius'), model.get('radiusUnits')) / map.getMap().getView().getProjection().getMetersPerUnit());
    return rectangle;
};
export const drawCircle = ({ map, model, rectangle, id, }: {
    map: any;
    model: any;
    rectangle: any;
    id: string;
    ...
    const point = Turf.point(translateFromOpenlayersCoordinate(rectangle.getCenter()));
    const turfCircle = new TurfCircle(point, rectangle.getRadius() *
        map.getMap().getView().getProjection().getMetersPerUnit(), 64, 'meters');
    const geometryRepresentation = new ol.geom.LineString(translateToOpenlayersCoordinates(turfCircle.geometry.coordinates[0]));
...
const useListenToBboxModel = ({ model, map }: {
    model: any;
    map: any;
}) => {
...
export const OpenlayersCircleDisplay = ({ map, model, }: {
    map: any;
    model: any;
}) => {
