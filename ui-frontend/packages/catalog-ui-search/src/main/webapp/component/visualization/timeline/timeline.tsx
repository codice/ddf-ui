import WithListenTo, { WithBackboneProps, } from './../../../react-component/backbone-container';
...
const getDateAttributes = (results: any) => {
    const availableAttributes = Object.keys(results)
        ...
        .sort();
    let dateAttributes = availableAttributes.reduce((list: any, attribute: any) => {
...
const renderTooltip = (timelineItems: TimelineItem[]) => {
    ...
    const results = timelineItems.slice(0, itemsToExpand).map((item) => {
        ...
        return (<React.Fragment key={metacard.id}>
...
const TimelineVisualization = (props: Props) => {
    ...
    React.useEffect(() => {
        const selectedIds = Object.values(selectedResults).map((result) => result.plain.metacard.properties.id);
        ...
        const resultData: TimelineItem[] = Object.values(results).map((result) => {
            ...
            possibleDateAttributes.forEach((dateAttribute: string) => {
                resultDateAttributes[dateAttribute] = moment(metacard[dateAttribute]) as Moment;
            });
    ...
    return (<TimelineWrapper data-id="timeline-container" ref={rootRef}>
      <Timeline min={moment('1975-01-01').tz(user.getTimeZone())} max={moment(maxDate)} heightOffset={250} onSelect={onSelect} data={data} dateAttributeAliases={dateAttributeAliases} renderTooltip={renderTooltip} format={user.getDateTimeFormat()} timezone={user.getTimeZone()} height={height} onCopy={(copiedValue) => {
