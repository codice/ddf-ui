...
export const getIdFromModelForDisplay = ({ model }: {
    model: any;
}) => {
    return `${SHAPE_ID_PREFIX}-${model.cid}-display`;
};
export const getIdFromModelForDrawing = ({ model }: {
    model: any;
}) => {
...
export const getLocationTypeFromModel = ({ model }: {
    model: any;
}) => {
    const type = model.get('type') as LocationTypeType;
    return type;
};
export const getDrawModeFromModel = ({ model, }: {
    model: any;
}): DrawModeType => {
...
const extractModelsFromFilter = ({ filter, extractedModels, }: {
    filter: any;
    extractedModels: any[];
}) => {
    if (filter.filters) {
...
function useOnceIsNearFirstRender({ howNear = 1000, callback, }: {
    howNear?: number;
    callback: () => void;
}) {
...
export const useDrawingAndDisplayModels = ({ selectionInterface, map, }: {
    ...
    useListenTo((wreqr as any).vent, 'search:linedisplay search:polydisplay search:bboxdisplay search:circledisplay search:keyworddisplay', (model: any) => {
        if (Array.isArray(model)) {
            setModels([...models].concat(model.filter((newModel) => !models.includes(newModel))));
        }
    ...
    useListenTo((wreqr as any).vent, 'search:drawline search:drawpoly search:drawbbox search:drawcircle', (model: any) => {
        if (!drawingModels.includes(model)) {
            setDrawingModels([...drawingModels, model]);
        }
    });
    useListenTo((wreqr as any).vent, 'search:drawline-end search:drawpoly-end search:drawbbox-end search:drawcircle-end', (model: any) => {
        if (drawingModels.includes(model)) {
            setDrawingModels(drawingModels.filter((drawingModel) => drawingModel !== model));
        }
    });
    useListenTo((wreqr as any).vent, 'search:drawend', (drawendModels: any[]) => {
        setDrawingModels(drawingModels.filter((drawingModel) => {
            return !drawendModels.includes(drawingModel);
        }));
        setModels(models.filter((drawingModel) => {
            return !drawendModels.includes(drawingModel);
        }));
    });
    React.useEffect(() => {
