...
const setCircleRadius = ({ model, map, mn, mx, }: {
    model: any;
    mn: any;
    mx: any;
    map: any;
    ...
    const radius = Math.abs(Cesium.Cartesian3.distance(startCartographic, stopCart));
    const modelProp = {
        lat: DistanceUtils.coordinateRound((mn.latitude * 180) / Math.PI),
        lon: DistanceUtils.coordinateRound((mn.longitude * 180) / Math.PI),
        radius: DistanceUtils.coordinateRound(DistanceUtils.getDistanceFromMeters(radius, model.get('radiusUnits'))),
    };
    model.set(modelProp);
};
...
export const CesiumCircleDrawing = ({ map, model, }: {
    map: any;
    model: any;
}) => {
