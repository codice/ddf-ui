...
function findExtreme({ objArray, property, comparator }: any) {
    ...
    return objArray.reduce((extreme: any, coordinateObj: any) => (extreme = comparator(extreme, coordinateObj[property])), objArray[0][property]);
}
...
function getHomeCoordinates() {
    if ((properties as any).mapHome !== '' && (properties as any).mapHome !== undefined) {
        const separateCoordinates = (properties as any).mapHome.replace(/\s/g, '').split(',');
