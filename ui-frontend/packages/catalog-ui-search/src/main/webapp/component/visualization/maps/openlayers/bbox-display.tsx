...
const modelToRectangle = (model: any) => {
    ...
    if (validateGeo('polygon', JSON.stringify([
        [west, north],
        [east, north],
        [west, south],
        [east, south],
    ...
    const northWest = ol.proj.transform([west, north], 'EPSG:4326', (properties as any).projection);
    const northEast = ol.proj.transform([east, north], 'EPSG:4326', (properties as any).projection);
    const southWest = ol.proj.transform([west, south], 'EPSG:4326', (properties as any).projection);
    const southEast = ol.proj.transform([east, south], 'EPSG:4326', (properties as any).projection);
    const coords = [];
...
export const drawBbox = ({ map, model, rectangle, id, }: {
    map: any;
    model: any;
    rectangle: any;
    id: string;
...
export const OpenlayersBboxDisplay = ({ map, model, }: {
    map: any;
    model: any;
}) => {
