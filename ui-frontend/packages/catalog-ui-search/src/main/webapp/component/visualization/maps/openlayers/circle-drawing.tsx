...
const setModelFromGeometry = ({ model, geometry, map, }: {
    model: any;
    geometry: any;
    map: any;
}) => {
    const center = translateFromOpenlayersCoordinate(geometry.getCenter());
    const rad = geometry.getRadius() *
    ...
    model.set({
        ...
        radius: DistanceUtils.coordinateRound(DistanceUtils.getDistanceFromMeters(rad, model.get('radiusUnits'))),
    });
};
    ...
    React.useEffect(() => {
        if (map && model) {
            ...
            primitive.on('drawend', (sketchFeature: any) => {
                ...
                (wreqr as any).vent.trigger('search:circledisplay', model);
                (wreqr as any).vent.trigger('search:drawcircle-end', model);
...
export const OpenlayersCircleDrawing = ({ map, model, }: {
    map: any;
    model: any;
}) => {
