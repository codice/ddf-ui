...
function calculateAvailableAttributes(results: LazyQueryResult[]) {
    ...
    return availableAttributes
        .filter((attribute) => metacardDefinitions.metacardTypes[attribute] !== undefined)
...
function findMatchesForAttributeValues(results: LazyQueryResult[], attribute: string, values: any[]) {
...
function getIndexClicked(data: any) {
    return Math.max.apply(undefined, data.points.map((point: any) => point.pointNumber)) as number;
}
...
const getAutocompleteState = ({ lazyResults, attributeToBin, }: {
...
export const Histogram = ({ selectionInterface }: Props) => {
    ...
    const [autocompleteState, setAutocompleteState] = React.useState(getAutocompleteState({ lazyResults, attributeToBin }));
            ...
            if (results.length > 0 && attributeToBin) {
                ...
                if (initialData[0].x.length === 0) {
                    ...
                    Plotly.newPlot(histogramElement, initialData, getLayout(), {
                        ...
                        Plotly.newPlot(histogramElement, determineData(plot), getLayout(plot), {
                            displayModeBar: false,
                        });
                        handleResize();
                        listenToHistogram();
    ...
    const selectBetween = (firstIndex: number, lastIndex: number) => {
        ...
        const validResults = validCategories.reduce((results: any, category: any) => {
            ...
            return results;
        }, [] as LazyQueryResult[]) as LazyQueryResult[];
    ...
    const handleControlClick = (data: any, alreadySelected: boolean) => {
        const attributeToCheck = attributeToBin;
        const categories = retrieveCategoriesFromPlotly();
        const matchedResults = findMatchesForAttributeValues(results, attributeToCheck, getValueFromClick(data, categories));
        if (alreadySelected) {
    ...
    const handleShiftClick = (data: any) => {
        ...
        const firstIndex = pointsSelected.current.length === 0
            ? -1
            : pointsSelected.current.reduce((currentMin, point) => Math.min(currentMin, point), pointsSelected.current[0]);
        const lastIndex = pointsSelected.current.length === 0
            ? -1
            : pointsSelected.current.reduce((currentMin, point) => Math.max(currentMin, point), pointsSelected.current[0]);
    ...
    const listenToHistogram = () => {
        if (plotlyRef.current) {
            const histogramElement = plotlyRef.current;
            (histogramElement as any)._ev.addListener('plotly_click', plotlyClickHandler);
        }
    };
    ...
    return (<>
      <div className="p-2">
        <Autocomplete size="small" options={autocompleteState.choices} onChange={(_e: any, newValue) => {
            setAttributeToBin(newValue.value);
        }} getOptionSelected={(option) => option.value === attributeToBin} getOptionLabel={(option) => {
            return option.label;
        }} disableClearable value={autocompleteState.choices.find((choice) => choice.value === attributeToBin)} renderInput={(params) => (<TextField {...params} label="Group by" variant="outlined"/>)}/>
      </div>
      <div className="plotly-histogram" ref={plotlyRef as any} style={{
            height: 'calc(100% - 135px)',
            width: '100%',
            display: noMatchingData ? 'none' : 'block',
        }}/>
      {noMatchingData ? (<div style={{ padding: '20px' }}>
