...
const removeOldDrawing = ({ map, model }: {
    map: ol.Map;
    model: any;
}) => {
    const oldLayer = map
...
const drawBorderedPolygon = ({ map, model, rectangle, }: {
    map: any;
    model: any;
    rectangle: any;
}) => {
...
const updatePrimitive = ({ map, model }: {
    map: any;
    model: any;
}) => {
...
export const OpenlayersLineDrawingStatic = ({ map, model, }: {
    map: any;
    model: any;
}) => {
...
const setModelFromGeometry = ({ model, geometry, }: {
    model: any;
    geometry: any;
}) => {
    model.set({
    ...
    React.useEffect(() => {
        if (map && model) {
            ...
            primitive.on('drawend', (sketchFeature: any) => {
                const geometry = OpenLayersGeometryUtils.wrapCoordinatesFromGeometry(sketchFeature.feature.getGeometry());
...
export const OpenlayersLineDrawingDynamic = ({ map, model, }: {
    map: any;
    model: any;
}) => {
...
export const OpenlayersLineDrawings = ({ map }: {
    map: any;
}) => {
    ...
    return (<>
      {models.map((model) => {
            return (<OpenlayersLineDrawingStatic key={model.id} model={model} map={map}/>);
        })}
      {drawingModels.map((model) => {
            return (<OpenlayersLineDrawingDynamic key={model.id} model={model} map={map}/>);
        })}
    </>);
};
