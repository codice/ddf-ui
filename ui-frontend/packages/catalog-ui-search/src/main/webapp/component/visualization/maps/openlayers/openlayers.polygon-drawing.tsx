  ...
  React.useEffect(() => {
    if (map && model) {
      ...
      primitive.on('drawend', (sketchFeature: any) => {
        ...
        wreqr.vent.trigger('search:drawline-end', model)
...
export const OpenlayersLineDrawings = ({ map }: { map: any }) => {
  ...
  useListenTo(wreqr.vent, 'search:linedisplay', (model: any) => {
    if (!models.includes(model)) {
      setModels([...models, model])
    }
  })
  useListenTo(wreqr.vent, 'search:drawline', (model: any) => {
    if (!drawingModels.includes(model)) {
      setDrawingModels([...drawingModels, model])
    }
  })
  useListenTo(wreqr.vent, 'search:drawline-end', (model: any) => {
    if (drawingModels.includes(model)) {
      setDrawingModels(
