...
export default function CesiumMap(insertionElement: any, selectionInterface: any, notificationEl: any, componentElement: any, mapModel: any) {
    let overlays = {};
    let shapes: any = [];
    ...
    function findOverlappingLabel(findSelected: any, geometry: any) {
        return _.find(mapModel.get('labels'), (label) => label.position.x === geometry.position.x &&
    ...
    function showHiddenLabel(geometry: any) {
        ...
        const hiddenLabel = _.find(mapModel.get('labels'), (label) => label.position.x === geometry.position.x &&
            label.position.y === geometry.position.y &&
    ...
    const exposedMethods = _.extend({}, Map, {
        ...
        onMouseTrackingForPopup(downCallback: any, moveCallback: any, upCallback: any) {
            map.screenSpaceEventHandlerForPopupPreview = new Cesium.ScreenSpaceEventHandler(map.canvas);
        ...
        doPanZoom(coords: any) {
            ...
            const cartArray = coords.map((coord: any) => Cesium.Cartographic.fromDegrees(coord[0], coord[1], map.camera._positionCartographic.height));
            if (cartArray.length === 1) {
                const point = Cesium.Ellipsoid.WGS84.cartographicToCartesian(cartArray[0]);
        ...
        panToResults(results: any) {
            ...
            cartArray = _.flatten(results
                ...
                .map((result: any) => _.map(result.getPoints('location'), (coordinate) => Cesium.Cartographic.fromDegrees(coordinate[0], coordinate[1], map.camera._positionCartographic.height)), true));
        ...
        panToShapesExtent() {
            const currentPrimitives = map.scene.primitives._primitives.filter((prim: any) => prim.id);
            const actualPositions = currentPrimitives.reduce((blob: any, prim: any) => {
                return blob.concat(prim._polylines.reduce((subblob: any, polyline: any) => {
        ...
        getBoundingBox() {
            ...
            return _.mapObject(viewRectangle, (val, key) => Cesium.Math.toDegrees(val));
        },
        overlayImage(model: LazyQueryResult) {
            ...
            const overlayLayer = map.scene.imageryLayers.addImageryProvider(new Cesium.SingleTileImageryProvider({
        ...
        getWindowLocationsOfResults(results: LazyQueryResult[]) {
            ...
            return results.map((result) => {
                const cartesian3CenterOfGeometry = utility.calculateCartesian3CenterOfGeometry(result);
                if (occluder && isNotVisible(cartesian3CenterOfGeometry, occluder)) {
                    return undefined;
                }
                const center = utility.calculateWindowCenterOfGeometry(cartesian3CenterOfGeometry, map);
                if (center) {
        ...
        addRulerLine(point: any) {
            ...
            return map.entities.add({
                polyline: {
                    ...
                    positions: new Cesium.CallbackProperty(function (time: any, result: any) {
        ...
        addPointWithText(point: any, options: any, useCustomText = false) {
            ...
            const cartographicPosition = Cesium.Cartographic.fromDegrees(pointObject.longitude, pointObject.latitude, pointObject.altitude);
            let cartesianPosition = map.scene.globe.ellipsoid.cartographicToCartesian(cartographicPosition);
        ...
        addPoint(point: any, options: any) {
            const pointObject = convertPointCoordinate(point);
            const cartographicPosition = Cesium.Cartographic.fromDegrees(pointObject.longitude, pointObject.latitude, pointObject.altitude);
            const cartesianPosition = map.scene.globe.ellipsoid.cartographicToCartesian(cartographicPosition);
            const billboardRef = billboardCollection.add({
                image: undefined,
                position: cartesianPosition,
                id: options.id,
        ...
        addLine(line: any, options: any) {
            ...
            const cartPoints = _.map(lineObject, (point) => Cesium.Cartographic.fromDegrees(point.longitude, point.latitude, point.altitude));
            const cartesian = map.scene.globe.ellipsoid.cartographicArrayToCartesianArray(cartPoints);
            ...
            if (map.scene.terrainProvider) {
                const promise = Cesium.sampleTerrain(map.scene.terrainProvider, 5, cartPoints);
                Cesium.when(promise, (updatedCartographic: any) => {
                    const positions = map.scene.globe.ellipsoid.cartographicArrayToCartesianArray(updatedCartographic);
                    if (updatedCartographic[0].height && !options.view.isDestroyed) {
        ...
        addPolygon(polygon: any, options: any) {
            const polygonObject = polygon.map((coordinate: any) => convertPointCoordinate(coordinate));
            const cartPoints = _.map(polygonObject, (point) => Cesium.Cartographic.fromDegrees(point.longitude, point.latitude, point.altitude));
            let cartesian = map.scene.globe.ellipsoid.cartographicArrayToCartesianArray(cartPoints);
