...
export const translateFromOpenlayersCoordinates = (coords: any) => {
    return coords
        .map((value: any) => value.map((point: any) => {
...
const coordsToLineString = (rawCoords: any) => {
    ...
    setArr.forEach((item: any) => {
        coords.push(ol.proj.transform([item[0], item[1]], 'EPSG:4326', (properties as any).projection));
    });
...
export const drawPolygon = ({ map, model, rectangle, id, }: {
    map: any;
    model: any;
    rectangle: any;
    id: string;
    ...
    coordinates.forEach((geometries, index) => (coordinates[index] = adjustPoints(geometries)));
    const bufferWidth = DistanceUtils.getDistanceInMeters(model.get('polygonBufferWidth'), model.get('polygonBufferUnits')) || 1;
    ...
    const bufferPolygonSegments = coordinates.map((set) => {
        ...
        const bufferPolygons = Turf.buffer(polySegment, bufferWidth, 'meters').geometry.coordinates.map((set: any) => {
            return Turf.polygon([set]);
        });
        return Turf.union(...bufferPolygons).geometry.coordinates;
    });
    const bufferGeometryRepresentation = (bufferPolygonSegments &&
        new ol.geom.MultiPolygon(bufferPolygonSegments)) ||
        coordinates;
    const drawnGeometryRepresentation = (drawnPolygonSegments && new ol.geom.MultiPolygon(drawnPolygonSegments)) ||
        coordinates;
...
const useListenToPolygonModel = ({ model, map }: {
    model: any;
    map: any;
}) => {
...
export const OpenlayersPolygonDisplay = ({ map, model, }: {
    map: any;
    model: any;
}) => {
