...
const handleShiftClick = ({ items, filteredItemArray, setItems, item, }: {
    items: CheckedType;
    item: string;
    ...
    if (firstIndex === defaultMin && lastIndex === defaultMax) {
        ...
        const updates = filteredItemArray.slice(closestPreviousIndex, indexClicked + 1);
        setItems({
            ...items,
            ...updates.reduce((blob, filteredItem) => {
                blob[filteredItem] = true;
...
const ItemRow = ({ value, lazyResult, startOver, measure, filter, }: {
    value: string;
    ...
    const { setItems, items, filteredItemArray } = React.useContext(CustomListContext);
    ...
    return (<div data-id="attribute-container" role="listitem" className="p-0 flex w-full">
      ...
      <Button fullWidth size="medium" className="children-block text-left" onClick={(event) => {
            if (event.shiftKey) {
                handleShiftClick({
                    items,
                    item: value,
      ...
      {!isReadonly && lazyResult && (<Button data-id="edit-button" style={{
                pointerEvents: 'all',
                height: '100%',
            }} onClick={() => {
                dialogContext.setProps({
                  ...
                  <Editor attr={value} lazyResult={lazyResult} 
...
const filterUpdate = ({ filter, setItemArray, items, }: {
    items: {
        [key: string]: boolean;
    };
    filter: string;
    ...
    setItemArray(Object.keys(items).filter((attr) => {
 ...
 */
const CustomList = ({ title, items, lazyResult, updateItems, isDnD, handleToggleAll, mode, startOver, totalPossible, }: {
    ...
    const [unfilteredItemArray, setUnfilteredItemArray] = React.useState(Object.keys(items));
    const [filteredItemArray, setFilteredItemArray] = React.useState(Object.keys(items));
    ...
    return (<CustomListContext.Provider value={memoProviderValue}>
      <Paper data-id={`${(title as any).toLowerCase()}-container`} elevation={Elevations.paper}>
        <Grid container className="p-2 text-xl font-normal relative" direction="row" wrap="nowrap" alignItems="center">
          <Grid item className="absolute left-0 top-0 ml-2 mt-min">
            <Button data-id={`${(title as any).toLowerCase()}-select-all-checkbox`} disabled={Object.keys(items).length === 0} onClick={handleToggleAll(items)} color={isIndeterminate || isCompletelySelected ? 'default' : 'default'}>
              {(() => {
        ...
        <div className="p-2">
          <TextField data-id="filter-input" size="small" variant="outlined" label="Filter by keyword" fullWidth={true} value={filter} onChange={(e) => {
            setFilter(e.target.value);
        }}/>
        </div>
        <DarkDivider className="w-full h-min"/>
        {mode === 'loading' ? (<CircularProgress />) : (<List className="w-common h-common overflow-hidden relative" dense component="div" role="list">
            {isFiltering ? (<LinearProgress className="w-full h-1 absolute left-0 top-0" variant="indeterminate"/>) : null}
            {isDnD ? (<Memo dependencies={[filteredItemArray]}>
                <div className="flex flex-col flex-no-wrap h-full w-full overflow-hidden">
                  ...
                  <div className="w-full h-full">
                    ...
                    if (result.reason === 'DROP' && result.destination) {
                        const shiftedOriginalIndex = unfilteredItemArray.indexOf(result.draggableId);
                        const shiftedDestIndex = unfilteredItemArray.indexOf(filteredItemArray[result.destination.index]);
                      ...
                      <div className="children-h-full children-w-full h-full w-full overflow-hidden">
                        <Droppable droppableId="test" mode="virtual" renderClone={(provided, _snapshot, rubric) => {
                    ...
                    return (<div {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef}>
                                <ItemRow value={filteredItemArray[rubric.source.index]} startOver={startOver} lazyResult={lazyResult} filter={filter}/>
                              </div>);
                }}>
                          {(provided) => {
                    return (<AutoVariableSizeList<string, HTMLDivElement> items={filteredItemArray} defaultSize={39.42} controlledMeasuring={true} overscanCount={10} outerRef={provided.innerRef} Item={({ itemRef, item, measure, index }) => {
                            return (<div ref={itemRef} className="relative">
                                      <Draggable draggableId={item} index={index} key={item} isDragDisabled={!isDnD} disableInteractiveElementBlocking>
                                        {(provided) => {
                                    ...
                                    return (<div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                              <ItemRow value={item} startOver={startOver} lazyResult={lazyResult} 
                                    // filter={filter}
                                    measure={measure}/>
                                            </div>);
                                }}
                ...
                <AutoVariableSizeList<string, HTMLDivElement> items={filteredItemArray} defaultSize={39.42} controlledMeasuring={true} overscanCount={10} Item={({ itemRef, item, measure }) => {
                    return (<div ref={itemRef} className="relative">
                        <ItemRow value={item} startOver={startOver} lazyResult={lazyResult} measure={measure}/>
                      </div>);
                }} Empty={() => {
                    return <div></div>;
                }}/>
...
export const useCustomReadOnlyCheck = () => {
    const [customEditableAttributes, setCustomEditableAttributes,] = React.useState([] as string[]);
...
const TransferList = ({ startingLeft, startingRight, lazyResult, onSave, }: {
    startingLeft: string[];
    ...
    const dialogContext = useDialog();
    const [mode, setMode] = React.useState('loading' as 'normal' | 'saving' | 'loading');
    ...
    const generateHandleToggleAll = ({ setState, state, }: {
        setState: SetCheckedType;
        state: CheckedType;
    }) => {
        return () => () => {
            if (Object.values(state).includes(false)) {
                setState(Object.keys(state).reduce((blob, attr) => {
                    blob[attr] = true;
                    return blob;
                }, {} as CheckedType));
            }
    ...
    const startOver = () => {
        dialogContext.setProps({
            open: true,
            children: (<TransferList startingLeft={Object.keys(left)} startingRight={Object.keys(right)} lazyResult={lazyResult} onSave={onSave}/>),
        });
    };
    ...
    return (<>
      <div className="text-2xl text-center px-2 pb-2 pt-4 font-normal relative">
        Manage Attributes
        <Button data-id="close-button" className="absolute right-0 top-0 mr-1 mt-1" variant="text" color="default" size="small" onClick={() => {
            dialogContext.setProps({
                open: false,
                children: null,
            });
      ...
      <DarkDivider className="w-full h-min"/>
      <DialogContent>
        <Grid container spacing={2} justify="center" alignItems="center" className="m-auto">
          <Grid item>
            <CustomList title="Active" items={left} lazyResult={lazyResult} updateItems={setLeft} isDnD={true} startOver={startOver} handleToggleAll={generateHandleToggleAll({
            setState: setLeft,
            state: left,
          ...
          <Grid item>
            <Grid container direction="column" alignItems="center">
              <Button data-id="move-right-button" variant="outlined" className="m-1" onClick={moveRight} disabled={!hasLeftChecked} aria-label="move selected right">
                <RightArrowIcon />
              </Button>
              <Button data-id="move-left-button" variant="outlined" className="m-1" onClick={moveLeft} disabled={!hasRightChecked} aria-label="move selected left">
                <LeftArrowIcon />
              </Button>
            </Grid>
          </Grid>
          <Grid item>
            <CustomList title="Hidden" items={right} lazyResult={lazyResult} updateItems={setRight} isDnD={false} startOver={startOver} handleToggleAll={generateHandleToggleAll({
            setState: setRight,
            state: right,
      ...
      <DarkDivider className="w-full h-min"/>
      <DialogActions>
        <Button data-id="dialog-save-button" disabled={mode === 'saving'} onClick={() => {
            dialogContext.setProps({
                open: false,
                children: null,
            });
        }} variant="text" color="secondary" className="mr-2">
          Cancel
        </Button>
        <Button className="ml-2" disabled={mode === 'saving'} onClick={() => {
            setMode('saving');
      ...
      {mode === 'saving' ? (<LinearProgress style={{
                width: '100%',
                height: '10px',
                position: 'absolute',
                left: '0px',
