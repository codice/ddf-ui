...
const ThumbnailInput = ({ value, onChange = () => { }, disabled = false, }: {
    value: string;
    disabled: boolean;
    ...
    return (<Grid container direction="row" alignItems="stretch" alignContent="stretch" wrap="nowrap">
      <Grid item style={{ overflow: 'hidden' }}>
        ...
        <a target="_blank" href={TypedMetacardDefs.getImageSrc({ val: value })} style={{ padding: '0px' }}>
          <img src={TypedMetacardDefs.getImageSrc({ val: value })} ref={imgRef} style={{ maxWidth: '100%', maxHeight: '50vh' }}/>
        </a>
      </Grid>
      <Grid item>
        <Button style={{ height: '100%' }} variant="outlined" disabled={disabled} onClick={() => {
...
const handleMetacardUpdate = ({ lazyResult, attributes, onSuccess, onFailure, }: {
...
export const Editor = ({ attr, lazyResult, onCancel = () => { }, onSave = () => { }, goBack, }: {
    attr: string;
    lazyResult: LazyQueryResult;
    onCancel?: () => void;
    onSave?: () => void;
    ...
    const [values, setValues] = React.useState(Array.isArray(lazyResult.plain.metacard.properties[attr])
    ...
    return (<>
      {goBack && (<Button variant="text" color="primary" startIcon={<KeyboardBackspaceIcon />} onClick={goBack}>
      ...
      <DialogContent style={{ minHeight: '30em', minWidth: '60vh' }}>
        {values.map((val: any, index: number) => {
            return (<Grid container direction="row" className="my-2">
              {index !== 0 ? <Divider style={{ margin: '5px 0px' }}/> : null}
              <Grid item md={11}>
                {(() => {
                    if (enumForAttr) {
                        return (<Autocomplete disabled={mode === 'saving'} value={val} onChange={(_e: any, newValue: string) => {
                                values[index] = newValue;
                                setValues([...values]);
                            }} fullWidth disableClearable size="small" options={enumForAttr} renderInput={(params) => (<TextField {...params} variant="outlined"/>)}/>);
                    }
                    switch (attrType) {
                        case 'DATE':
                            return (<DateTimePicker value={val} onChange={(value) => {
                                    values[index] = value;
                                    setValues([...values]);
                                }} TextFieldProps={{
                        ...
                        case 'BINARY':
                            return (<ThumbnailInput disabled={mode !== Mode.Normal} value={val} onChange={(update) => {
                                    values[index] = update;
                                    setValues([...values]);
                                }}/>);
                        case 'BOOLEAN':
                            return (<Checkbox disabled={mode !== Mode.Normal} checked={val} onChange={(e) => {
                        ...
                        case 'INTEGER':
                        case 'SHORT':
                            return (<TextField disabled={mode !== Mode.Normal} value={val} onChange={(e) => {
                                    values[index] = e.target.value;
                                    setValues([...values]);
                                }} type="number" fullWidth/>);
                        case 'GEOMETRY':
                            return (<LocationInputReact onChange={(location: any) => {
                        ...
                        default:
                            return (<TextField disabled={mode !== Mode.Normal} value={val} onChange={(e: any) => {
                                    values[index] = e.target.value;
                                    setValues([...values]);
                                }} style={{ whiteSpace: 'pre-line', flexGrow: 50 }} fullWidth multiline={true} rowsMax={1000}/>);
                    }
                })()}
              </Grid>
              {isMultiValued ? (<Grid item md={1}>
                  <Button disabled={mode === Mode.Saving} onClick={() => {
        ...
        {isMultiValued && values.length > 0 && (<Button disabled={mode === Mode.Saving} variant="text" color="primary" onClick={() => {
                let defaultValue = '';
                switch (attrType) {
                    case 'DATE':
      ...
      <Divider />
      <DialogActions>
        <Button disabled={mode === Mode.Saving} variant="text" onClick={() => {
            onCancel();
        }}>
          Cancel
        </Button>
        <Button disabled={mode !== Mode.Normal} variant="contained" color="primary" onClick={() => {
            setMode(Mode.Saving);
            ...
            const onSuccess = () => setTimeout(() => {
                addSnack('Successfully updated.');
                onSave();
            }, 1000);
            const onFailure = () => setTimeout(() => {
      ...
      {mode === Mode.Saving ? (<LinearProgress style={{
                width: '100%',
                height: '10px',
                position: 'absolute',
                left: '0px',
...
const AttributeComponent = ({ lazyResult, attr, summaryShown = [], filter = '', forceRender, }: {
    attr: string;
    lazyResult: LazyQueryResult;
    summaryShown?: string[];
    filter?: string;
    ...
    const isFiltered = filter !== '' ? !label.toLowerCase().includes(filter.toLowerCase()) : false;
    const MemoItem = React.useMemo(() => {
        return (<Grid container direction="row" wrap={'nowrap'} className="group relative">
        {isNotWritable({ attribute: attr, lazyResult }) ? null : (<div className="p-1 hidden group-hover:block absolute right-0 top-0">
            <Button onClick={() => {
                    dialogContext.setProps({
                        open: true,
                        disableEnforceFocus: true,
                        children: (<Editor attr={attr} lazyResult={lazyResult} onCancel={() => {
                                dialogContext.setProps({
                                    open: false,
                                    children: null,
                                });
        ...
        <Grid item xs={4} style={{
          ...
          <Divider orientation="vertical" className="absolute right-0 top-0 w-min h-full"/>
        </Grid>
                    ...
                    <div>
                      {(() => {
                        if (attr === 'ext.audio-snippet') {
                            const mimetype = lazyResult.plain.metacard.properties['ext.audio-snippet-mimetype'];
                            const src = `data:${mimetype};base64,${val}`;
                            return <audio controls src={src}/>;
                        }
                        switch (TypedMetacardDefs.getType({ attr })) {
                            case 'DATE':
                                return (<Typography title={Common.getMomentDate(val)}>
                                {user.getUserReadableDateTime(val)}
                              </Typography>);
                            case 'BINARY':
                                return (<a target="_blank" href={TypedMetacardDefs.getImageSrc({ val })} style={{ padding: '0px' }}>
                                <img src={TypedMetacardDefs.getImageSrc({ val })} style={{
                                        maxWidth: '100%',
                                        maxHeight: '50vh',
                                    }}/>
                              </a>);
                            ...
                            case 'GEOMETRY':
                                return (<Typography>{convertToFormat(val)}</Typography>);
                            default:
                                if (lazyResult.highlights[attr]) {
                                    ...
                                    {
                                        return isUrl(val) ? (<Typography>
                                    <span className="highlight">
                                      <a href={val} target="_blank">
                                        {val}
                                      </a>
                                    </span>
                                  </Typography>) : (displayHighlightedAttrInFull(lazyResult.highlights[attr], val, index));
                                    }
                                }
                                else if (isUrl(val)) {
                                    return (<Typography>
    ...
    return (<div style={{ display: isFiltered ? 'none' : 'block' }}>{MemoItem}</div>);
};
...
const getHiddenAttributes = (selection: LazyQueryResult, activeAttributes: string[]) => {
...
const Summary = ({ result: selection }: Props) => {
    ...
    React.useEffect(() => {
        listenTo(user.get('user').get('preferences'), 'change:inspector-summaryShown change:dateTimeFormat change:timeZone', () => {
    ...
    return (<Grid container direction="column" wrap="nowrap" className="overflow-hidden w-full h-full">
      <Grid item className="flex-shrink-0">
        <Grid container direction="row" alignItems="center" wrap="nowrap" justify="space-between" className="p-2">
          <Grid item>
            <Button data-id="manage-attributes-button" onClick={() => {
            dialogContext.setProps({
                PaperProps: {
                    style: {
                        minWidth: 'none',
                ...
                children: (<div style={{
                      ...
                      <TransferList startingLeft={summaryShown} startingRight={getHiddenAttributes(selection, summaryShown)
                        .map((attr) => {
                        return attr.id;
                    })
                        .sort()} lazyResult={selection} onSave={(active) => {
                        user
                            .get('user')
                            .get('preferences')
        ...
        }} color="primary" size="small" style={{ height: 'auto' }}>
              Manage Attributes
            </Button>
          </Grid>

          <Grid item>
            <TextField data-id="summary-filter-input" size="small" variant="outlined" label="Filter" value={filter} inputProps={{
            style: filter !== ''
                ? {
      ...
      <Grid item className="flex-shrink-1 overflow-auto p-2">
        <Paper elevation={Elevations.paper}>
          {summaryShown.map((attr, index) => {
            return (<div className="relative" key={attr}>
                <AttributeComponent lazyResult={selection} attr={attr} summaryShown={summaryShown} filter={filter} forceRender={forceRender}/>
                {index !== 0 ? (<Divider orientation="horizontal" className="absolute top-0 w-full h-min"/>) : null}
              </div>);
        })}

          {expanded ? (<>
              {everythingElse.map((attr) => {
                return (<div key={attr} className="relative">
                    <AttributeComponent lazyResult={selection} attr={attr} summaryShown={summaryShown} filter={filter} forceRender={forceRender}/>
                    <Divider orientation="horizontal" className="absolute top-0 w-full h-min"/>
                  </div>);
            })}
              {blankEverythingElse.map((attr) => {
                return (<div key={attr.id} className="relative">
                    <AttributeComponent lazyResult={selection} attr={attr.id} summaryShown={summaryShown} filter={filter} forceRender={forceRender}/>
                    <Divider orientation="horizontal" className="absolute top-0 w-full h-min"/>
                  </div>);
            })}
            </>) : (<></>)}
      ...
      {!fullyExpanded && (<>
          ...
          <Grid item className="flex-shrink-0 p-2">
            <Button data-id="see-all-collapse-button" onClick={() => {
