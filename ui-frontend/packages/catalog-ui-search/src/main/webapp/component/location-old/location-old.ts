...
export default Backbone.AssociatedModel.extend({
    ...
    repositionLatLon() {
        ...
        this.repositionLatLonUtmUps((_this: any) => _this.isUtmUpsUpperLeftDefined(), (_this: any) => _this.parseUtmUpsUpperLeft(), (newResult: any, lat: any, lon: any) => {
            newResult.mapNorth = lat;
            newResult.mapWest = lon;
        }, (_this: any) => _this.clearUtmUpsUpperLeft(true));
        this.repositionLatLonUtmUps((_this: any) => _this.isUtmUpsLowerRightDefined(), (_this: any) => _this.parseUtmUpsLowerRight(), (newResult: any, lat: any, lon: any) => {
            newResult.mapSouth = lat;
            newResult.mapEast = lon;
        }, (_this: any) => _this.clearUtmUpsLowerRight(true));
    },
    setLatLonUtmUps(result: any, isDefined: any, parse: any, assign: any, clear: any) {
    ...
    convertLatLonLinePolyToUtm(points: any) {
        return points
            ? points.map((point: any) => {
                ...
                return {
                    ...llPoint,
                    hemisphere: llPoint.hemisphere.toUpperCase() === 'NORTHERN'
                        ? 'Northern'
                        : 'Southern',
                };
            })
    ...
    setLatLonLinePolyFromUtmUps() {
        ...
        if (key === 'poly')
            key = 'polygon';
        if (key && (key === 'line' || key === 'polygon')) {
            let validation = validateUtmUpsLineOrPoly(this.get('utmUpsPointArray'), key);
            if (!validation.error) {
    ...
    setLatLon() {
        if (this.get('locationType') === 'dd') {
            ...
            if (!((result as any).north !== undefined &&
            ...
            this.setLatLonUtmUps(result, (_this: any) => _this.isUtmUpsUpperLeftDefined(), (_this: any) => _this.parseUtmUpsUpperLeft(), (result: any, lat: any, lon: any) => {
                result.north = lat;
                result.west = lon;
            }, (_this: any) => {
                _this.clearUtmUpsUpperLeft(true);
            });
            this.setLatLonUtmUps(result, (_this: any) => _this.isUtmUpsLowerRightDefined(), (_this: any) => _this.parseUtmUpsLowerRight(), (result: any, lat: any, lon: any) => {
                result.south = lat;
                result.east = lon;
            }, (_this: any) => {
                _this.clearUtmUpsLowerRight(true);
            });
            (result as any).north = DistanceUtils.coordinateRound((result as any).north);
            (result as any).east = DistanceUtils.coordinateRound((result as any).east);
            (result as any).south = DistanceUtils.coordinateRound((result as any).south);
            (result as any).west = DistanceUtils.coordinateRound((result as any).west);
            this.set(result);
        }
    ...
    setBboxLatLon() {
        const north = parseFloat(this.get('north')), south = parseFloat(this.get('south')), west = parseFloat(this.get('west')), east = parseFloat(this.get('east'));
    ...
    setLatLonFromDms(dmsCoordinateKey: any, dmsDirectionKey: any, latLonKey: any) {
        const dmsCoordinate = dmsUtils.parseDmsCoordinate(this.get(dmsCoordinateKey));
        if (dmsCoordinate) {
            this.set(latLonKey, dmsUtils.dmsCoordinateToDD({
                ...dmsCoordinate,
    ...
    parseUtmUps(eastingRaw: any, northingRaw: any, zoneRaw: any, hemisphereRaw: any) {
        const easting = parseFloat(eastingRaw);
        const northing = parseFloat(northingRaw);
        const zone = parseInt(zoneRaw);
        const hemisphere = hemisphereRaw === 'Northern'
            ? 'NORTHERN'
