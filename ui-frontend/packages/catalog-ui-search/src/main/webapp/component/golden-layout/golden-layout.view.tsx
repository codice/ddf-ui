...
const sanitizeTree = (tree: any) => treeMap(tree, (obj: any) => {
...
const GoldenLayoutComponentHeader = ({ viz, tab, options, componentState, container, name, }: {
    viz: any;
    tab: any;
    options: any;
    componentState: any;
    ...
    return (<ExtensionPoints.providers>
      <div data-id={`${name}-tab`} className={`flex flex-row items-center flex-nowrap ${isMinimized ? 'hidden' : ''}`}>
        <Grid item className="px-2 text-lg">
          <div>{tab.titleElement.text()}</div>
        </Grid>
        <Grid item>
          {viz.header ? (<viz.header {..._.extend({}, options, componentState, {
            container,
        })}/>) : null}
        </Grid>
        <Grid item>
          {tab.closeElement[0].style.display !== 'none' ? (<Button data-id="close-tab-button" onClick={(e) => {
...
const GoldenLayoutComponent = ({ ComponentView, options, container, }: {
    options: any;
    ComponentView: any;
    container: any;
}) => {
    ...
    const isMinimized = width <= 100;
    return (<ExtensionPoints.providers>
      <Paper elevation={Elevations.panels} className={`w-full h-full ${isMinimized ? 'hidden' : ''}`} square>
...
function registerComponent(marionetteView: any, name: any, ComponentView: any, componentOptions: any, viz: any) {
    const options = _.extend({}, marionetteView.options, componentOptions);
    marionetteView.goldenLayout.registerComponent(name, (container: any, componentState: any) => {
        container.on('open', () => {
            setTimeout(() => {
                ReactDOM.render(<GoldenLayoutComponent options={options} ComponentView={ComponentView} container={container}/>, container.getElement()[0]);
        ...
        container.on('tab', (tab: any) => {
            ...
            let intervalId = setInterval(() => {
                try {
                    ReactDOM.render(<GoldenLayoutComponentHeader viz={viz} tab={tab} options={options} componentState={componentState} container={container} name={name}/>, tab.element[0]);
...
//   })
// }
function getGoldenLayoutConfig({ layoutResult, editLayoutRef, configName, }: GoldenLayoutViewProps) {
...
function registerGoldenLayoutComponents({ goldenLayout, options, }: {
    goldenLayout: any;
    options: GoldenLayoutViewProps;
}) {
    Visualizations.forEach((viz) => {
        try {
            registerComponent({ goldenLayout, options }, viz.id, viz.view, viz.options, viz);
        }
        catch (err) {
...
export function getInstanceConfig({ goldenLayout }: {
    goldenLayout: any;
}) {
    const currentConfig = goldenLayout.toConfig();
    return removeEphemeralState(currentConfig);
}
function handleGoldenLayoutStateChange({ options, goldenLayout, lastConfig, }: {
    goldenLayout: any;
    options: GoldenLayoutViewProps;
    ...
    if (_.isEqual(removeEphemeralState(lastConfig.current), removeEphemeralState(getInstanceConfig({ goldenLayout })))) {
        return;
    }
    lastConfig.current = getInstanceConfig({ goldenLayout });
    /**
...
function handleGoldenLayoutStackCreated(stack: any) {
    let intervalId = setInterval(() => {
        try {
            ReactDOM.render(<GoldenLayoutToolbar stack={stack}/>, stack.header.controlsContainer[0]);
            clearInterval(intervalId);
        }
        catch (err) { }
    }, 100);
}
const GoldenLayoutToolbar = ({ stack }: {
    stack: any;
}) => {
      ...
      <Grid container direction="row" wrap="nowrap">
        {isMinimized ? (<>
            ...
            <Grid item>
              ...
              <Button data-id="maximise-tab-button" onClick={(e) => {
            ...
            <Grid item>
              {/* @ts-expect-error ts-migrate(6133) FIXME: 'e' is declared but its value is never read. */}
              <Button data-id="minimise-layout-button" onClick={(e) => {
            ...
            <Grid item>
              {/* @ts-expect-error ts-migrate(6133) FIXME: 'e' is declared but its value is never read. */}
              <Button data-id="maximise-layout-button" onClick={(e) => {
            ...
            <Grid item>
              {/* @ts-expect-error ts-migrate(6133) FIXME: 'e' is declared but its value is never read. */}
              {stack.header._isClosable() ? (<Button data-id="close-layout-button" onClick={(e) => {
...
export const GoldenLayoutViewReact = (options: GoldenLayoutViewProps) => {
    const [goldenLayoutAttachElement, setGoldenLayoutAttachElement,] = React.useState<HTMLDivElement | null>(null);
    ...
    React.useEffect(() => {
        ...
        $(window).on('resize.' + randomString, _debounce((event: any) => {
            goldenLayout.updateSize();
        }, 100, {
            leading: false,
            trailing: true,
    ...
    React.useEffect(() => {
        if (goldenLayout) {
            ...
            goldenLayout.on('stateChanged', _.debounce(() => {
    ...
    return (<div data-element="golden-layout" className="is-minimised h-full w-full">
      <div ref={setGoldenLayoutAttachElement} className="golden-layout-container w-full h-full"/>
    </div>);
};
