...
export function useIsTruncated<T extends HTMLElement>(passedInRef: T | null = null) {
...
const OverflowTip = ({ children, tooltipProps = {}, refOfThingToMeasure: refOfThingToMeasurePassedIn, className, }: OverflowTipType) => {
    ...
    return (<Tooltip title={<Paper className="p-1 overflow-auto max-w-screen-sm" elevation={Elevations.overlays}>
          {title ? title : children}
        </Paper>} open={open} onOpen={() => {
            if (isTruncatedState.isTruncated) {
                setOpen(true);
            }
        }} onClose={() => {
