...
const PropertyComponent = (props: React.AllHTMLAttributes<HTMLDivElement>) => {
    return (<div {...props} className="overflow-auto" style={{
            marginTop: '10px',
            opacity: '.7',
            maxHeight: '200px',
...
const MultiSelectActions = ({ selectionInterface, }: {
    selectionInterface: any;
}) => {
    ...
    return (<>
      <Button className={selectedResultsArray.length === 0 ? 'relative' : 'relative'} color="primary" disabled={selectedResultsArray.length === 0} onClick={(e) => {
            e.stopPropagation();
      ...
      <Popover {...metacardInteractionMenuState.MuiPopoverProps}>
        <Paper>
          <LazyMetacardInteractions lazyResults={selectedResultsArray} onClose={metacardInteractionMenuState.handleClose}/>
        </Paper>
      </Popover>
    </>);
};
const dynamicActionClasses = 'h-full';
const DynamicActions = ({ lazyResult }: {
    lazyResult: LazyQueryResult;
}) => {
    ...
    const metacardInteractionMenuState = useMenuState();
    return (<Grid container direction="column" wrap="nowrap" alignItems="center">
      <Grid item className="h-full">
        <Button data-id="result-item-more-vert-button" onClick={(e) => {
            e.stopPropagation();
            metacardInteractionMenuState.handleClick();
        }} style={{ height: '100%' }} size="small" innerRef={metacardInteractionMenuState.anchorRef}>
          <MoreIcon />
        </Button>
        <Popover {...metacardInteractionMenuState.MuiPopoverProps} keepMounted={true}>
          <Paper>
            <LazyMetacardInteractions lazyResults={[lazyResult]} onClose={metacardInteractionMenuState.handleClose}/>
          </Paper>
        </Popover>
      </Grid>
      <Grid item className={dynamicActionClasses}>
        {lazyResult.hasErrors() ? (<div data-id="validation-errors-icon" className="h-full" title="Has validation errors." data-help="Indicates the given result has a validation error.
                     See the 'Quality' tab of the result for more details.">
            <WarningIcon />
          </div>) : ('')}
      </Grid>
      <Grid item className={dynamicActionClasses}>
        {!lazyResult.hasErrors() && lazyResult.hasWarnings() ? (<div data-id="validation-warnings-icon" className="h-full" title="Has validation warnings." data-help="Indicates the given result has a validation warning.
                     See the 'Quality' tab of the result for more details.">
            <WarningIcon />
          </div>) : ('')}
      </Grid>
      <Grid item className={dynamicActionClasses}>
        {lazyResult.plain.metacard.properties['ext.link'] ? (<Button title={lazyResult.plain.metacard.properties['ext.link']} onClick={(e) => {
                e.stopPropagation();
      ...
      <Grid item className={dynamicActionClasses}>
        {lazyResult.isDownloadable() ? (<Button data-id="download-button" onClick={(e) => {
                e.stopPropagation();
                triggerDownload(e);
            }} style={{ height: '100%' }} size="small">
            <GetAppIcon />
      ...
      <Grid item className={dynamicActionClasses}>
        {lazyResult.isSearch() ? (<Link component={Button} data-id="edit-button" to={`/search/${lazyResult.plain.id}`} style={{ height: '100%' }}>
            <EditIcon />
          </Link>) : null}
      </Grid>
...
export const SelectionBackground = ({ lazyResult, style, }: {
    lazyResult: LazyQueryResult;
    style?: React.CSSProperties;
}) => {
    const isSelected = useSelectionOfLazyResult({ lazyResult });
    return (<div className="absolute left-0 top-0 z-0 w-full h-full Mui-bg-secondary" style={{
            opacity: isSelected ? 0.05 : 0,
            ...style,
        }}/>);
};
const IconButton = ({ lazyResult }: {
    lazyResult: LazyQueryResult;
}) => {
    const isSelected = useSelectionOfLazyResult({ lazyResult });
    return (<Button data-id="select-checkbox" onClick={(event) => {
        ...
        }} focusVisibleClassName="focus-visible" className="relative p-2 min-w-0 outline-none h-full group-1 flex-shrink-0">
      {(() => {
            if (isSelected) {
                return (<>
              <div className={`absolute w-full h-full left-0 top-0 opacity-0 transform transition duration-200 ease-in-out -translate-x-full`}>
              ...
              <div className={`transform transition duration-200 ease-in-out -translate-x-full group-1-focus-visible:translate-x-0 group-1-hover:translate-x-0`}>
                <CheckBoxIcon className="group-1-hover:block group-1-focus-visible:block hidden"/>
                <CheckIcon className="group-1-hover:hidden group-1-focus-visible:hidden block"/>
              </div>
            </>);
              ...
              <CheckBoxOutlineBlankIcon className={`group-1-hover:visible group-1-focus-visible:visible invisible`}/>
            </div>);
            }
            return null;
        })()}
...
const diagonalHoverClasses = 'absolute z-50 right-0 bottom-100 h-4 transform scale-0 group-hover:scale-100 transition-all absolute z-50 right-0 bottom-100';
...
export const ResultItem = ({ lazyResult, measure }: ResultItemFullProps) => {
    ...
    const [shownAttributes, setShownAttributes] = React.useState(TypedUserInstance.getResultsAttributesShownList());
    useRerenderOnBackboneSync({ lazyResult });
    React.useEffect(() => {
        listenTo(user.get('user').get('preferences'), 'change:results-attributesShownList', () => {
    ...
    const extraHighlights = Object.keys(lazyResult.highlights).filter((attr) => !shownAttributes.find((shownAttribute) => shownAttribute === attr));
    const getDisplayValue = ({ detail, lazyResult, }: {
        detail: string;
        lazyResult: LazyQueryResult;
    }) => {
    ...
    return (<button data-id="result-item-container-button" onMouseDown={(event: any) => {
            /**
      ...
      <div className="w-full">
        ...
        <div className="w-full relative z-0">
          <div className="w-full flex items-start">
            ...
            <div data-id={`result-item-${shownAttributes[0]}-label`} title={`${TypedMetacardDefs.getAlias({
            attr: shownAttributes[0],
        })}`} className="flex-shrink-1 w-full overflow-auto self-center" style={{ maxHeight: '200px', minHeight: '21px' }} // firefox will show scrollbars always without this minHeight :S
    >
              {shownAttributes[0] === 'thumbnail' && thumbnail ? (<img data-id="result-item-thumbnail" src={imgsrc} style={{ maxWidth: '100%', maxHeight: '200px' }} onLoad={() => {
                measure();
            }} onError={() => {
                measure();
            }}/>) : lazyResult.highlights[shownAttributes[0]] ? (<span dangerouslySetInnerHTML={{
                __html: lazyResult.highlights[shownAttributes[0]][0].highlight,
            }}/>) : (getDisplayValue({ detail: shownAttributes[0], lazyResult }))}
            </div>
          </div>
          <div className={`pl-3 ${ResultItemAddOnInstance !== null ||
            detailsMap.length > 0 ||
            ...
            if (detail.attribute === 'thumbnail') {
                return (<img data-id="result-item-thumbnail" src={imgsrc} style={{
                        marginTop: '10px',
                        maxWidth: '100%',
                        maxHeight: '200px',
                    }} onLoad={() => {
            ...
            return (<PropertyComponent key={detail.attribute} data-help={TypedMetacardDefs.getAlias({
                    ...
                    <span>
                      ...
                      {lazyResult.highlights[detail.attribute] ? (<span dangerouslySetInnerHTML={{
            ...
            const relevantHighlight = lazyResult.highlights[extraHighlight][0];
            return (<PropertyComponent key={relevantHighlight.attribute} data-help={TypedMetacardDefs.getAlias({
              ...
              {shouldShowRelevance ? (<PropertyComponent data-help={`Relevance: ${lazyResult.plain.relevance}`} title={`Relevance: ${lazyResult.plain.relevance}`}>
                  <span>{lazyResult.getRoundedRelevance()}</span>
                </PropertyComponent>) : ('')}
              {shouldShowSource ? (<PropertyComponent title={`${TypedMetacardDefs.getAlias({
                attr: 'source-id',
            })}: ${lazyResult.plain.metacard.properties['source-id']}`} data-help={TypedMetacardDefs.getAlias({
                attr: 'source-id',
            })}>
                  {!lazyResult.isRemote() ? (<React.Fragment>
        ...
        {renderExtras ? (<>
            ...
            <div className={`${diagonalHoverClasses} w-full transform translate-y-1`}/>
            <div className={`${diagonalHoverClasses} w-9/12 transform translate-y-2 `}/>
            <div className={`${diagonalHoverClasses} w-6/12 transform translate-y-3`}/>
            <div className={`${diagonalHoverClasses} w-5/12 transform translate-y-4`}/>
            <div className={`${diagonalHoverClasses} w-4/12 transform translate-y-5`}/>
            <div className={`${diagonalHoverClasses} w-3/12 transform translate-y-6`}/>
            <div className={`${diagonalHoverClasses} w-2/12 transform translate-y-8`}/>
            <div className={`absolute z-40 group-hover:z-50 focus-within:z-50 right-0 top-0 focus-within:opacity-100 group-hover:opacity-100 hover:opacity-100 opacity-0 cursor-auto transform focus-within:scale-100 transition-all hover:scale-100 ease-in-out duration-200 hover:translate-x-0 hover:scale-x-100`}>
              <Paper onClick={(e) => {
                e.stopPropagation();
