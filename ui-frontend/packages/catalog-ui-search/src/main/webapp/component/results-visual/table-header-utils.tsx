...
const defaultTableColumns = (properties as any).defaultTableColumns
    ? (properties as any).defaultTableColumns.map((attr: string) => attr.toLowerCase())
    : [];
const setDefaultColumns = (filteredAttributes: any) => {
    ...
    if (!hasSelectedColumns &&
        ...
        const hiddenAttributes = availableAttributes.filter((attr: string) => !defaultTableColumns.includes(attr.toLowerCase()));
...
export const getVisibleHeaders = (filteredAttributes: any) => {
    const sortAttributes = _.filter(metacardDefinitions.sortedMetacardTypes, (type: any) => !metacardDefinitions.isHiddenTypeExceptThumbnail(type.id)).map((type: any) => type.id);
