...
export const ResultsCommonControls = ({ getStartingLeft, getStartingRight, onSave, }: ResultsCommonControlsType) => {
        ...
        <Button data-id="manage-attributes-button" onClick={() => {
            dialogContext.setProps({
                ...
                children: (<div style={{
                  ...
                  <TransferList startingLeft={getStartingLeft()} startingRight={getStartingRight()} onSave={(active) => {
                        onSave(active);
                    }}/>
                </div>),
            });
...
const TableVisual = ({ selectionInterface, mode, setMode }: Props) => {
    ...
    return (<Grid container className="w-full h-full bg-inherit" direction="column" wrap="nowrap">
      <Grid item>
        <Grid container className="w-full pt-2 px-2" direction="row" alignItems="center">
          <ResultsCommonControls getStartingLeft={() => {
          ...
          <Grid item className="pr-2">
            <Button data-id="list-button" onClick={() => {
            setMode('card');
        }} style={{
          ...
          <Grid item>
            <Button data-id="table-button" onClick={() => {
            setMode('table');
        }} style={{
      ...
      <Grid item className="overflow-hidden bg-inherit w-full h-full p-2">
        <Paper elevation={Elevations.paper} className="w-full h-full">
          {isMounted ? (<Grid container className="w-full h-full bg-inherit" direction="column" wrap="nowrap">
              <Grid item className="bg-inherit">
                <div className="w-auto overflow-auto scrollbars-hide bg-inherit" ref={headerRef}>
              ...
              <Grid item className="w-full h-full overflow-hidden bg-inherit">
                <AutoVariableSizeList<LazyQueryResult, HTMLDivElement> outerElementProps={{
                onScroll: (e) => {
                    if (headerRef.current) {
                        headerRef.current.scrollLeft = (e.target as any).scrollLeft;
                    }
                },
            }} defaultSize={76} overscanCount={10} controlledMeasuring={true} items={results} Item={({ itemRef, item, measure, index }) => {
                return (<div ref={itemRef} className="bg-inherit">
                        <ResultItemRow lazyResult={item} measure={measure} index={index} results={results}/>
                      </div>);
            }} Empty={() => {
                ...
                return (<div className="result-item-collection-empty p-2">
                        No Results Found
                      </div>);
            }}/>
              </Grid>
            </Grid>) : (<LinearProgress variant="indeterminate"/>)}
        </Paper>
      </Grid>
    </Grid>);
};
