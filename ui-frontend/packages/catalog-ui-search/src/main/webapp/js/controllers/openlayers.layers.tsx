...
const createTile = ({ show, alpha, ...options }: any, Source: any, Layer = ol.layer.Tile) => new Layer({
    visible: show,
    preload: Infinity,
    opacity: alpha,
...
const SI = (opts: any) => {
    const imageExtent = opts.imageExtent || ol.proj.get((properties as any).projection).getExtent();
...
export class OpenlayersLayers extends Subscribable<''> {
    ...
    makeMap(mapOptions: MakeMapType) {
        ...
        const view = new ol.View({
            ...
            center: ol.proj.transform([0, 0], 'EPSG:4326', (properties as any).projection),
