...
function checkValidation(model: any) {
    if (model.get('id')) {
        ...
        setTimeout(function (this: any) {
            ($ as any).whenAll
                .apply(this, [
                $.get('./internal/metacard/' + model.get('id') + '/attribute/validation').then((response) => {
                    model.set({
                        issues: model.get('issues') || response.length > 0,
                    });
                }),
                $.get('./internal/metacard/' + model.get('id') + '/validation').then((response) => {
                    model.set({
                        issues: model.get('issues') || response.length > 0,
                    });
                }),
...
export default Backbone.Model.extend({
    ...
    handleUploadProgress(file: any, percentage: any) {
        if (fileMatches(file, this)) {
