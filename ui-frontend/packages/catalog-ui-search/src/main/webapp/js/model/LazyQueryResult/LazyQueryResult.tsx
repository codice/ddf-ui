...
function getThumbnailAction(result: ResultType) {
    return result.actions.find((action) => action.id === 'catalog.data.metacard.thumbnail');
}
...
export class LazyQueryResult {
    ...
    _notifySubscribers(subscribableThing: SubscribableType) {
        // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        const subscribers = this[`subscriptionsToMe.${subscribableThing}`] as SubscriptionType;
    ...
    refreshFromEditResponse(response: [
        ...
        response.forEach((part) => part.attributes.forEach((attribute) => {
            this.plain.metacard.properties[attribute.attribute] = TypedMetacardDefs.isMulti({ attr: attribute.attribute })
                ? attribute.values
                : attribute.values[0];
        }));
        this.syncWithPlain();
    ...
    refreshData(metacardProperties: LazyQueryResult['plain']['metacard']['properties']) {
                        ...
                        new FilterBuilderClass({
                            ...
                            filters: [
                                new FilterClass({
                                    type: '=',
                                    property: '"id"',
                                    value: this.plain.metacard.properties['metacard.deleted.id'] ||
                                        this.plain.id,
                                }),
                                new FilterClass({
                                    type: '=',
    ...
    isRemote(): boolean {
        return (Sources.getHarvested().includes(this.plain.metacard.properties['source-id']) === false);
    }
    hasGeometry(attribute?: any): boolean {
        return (_.filter(this.plain.metacard.properties, (_value: any, key: string) => (attribute === undefined || attribute === key) &&
            metacardDefinitions.metacardTypes[key] &&
            metacardDefinitions.metacardTypes[key].type === 'GEOMETRY').length > 0);
    }
    getGeometries(attribute?: any): any {
        return _.filter(this.plain.metacard.properties, (_value: any, key: string) => !properties.isHidden(key) &&
    ...
    getPoints(attribute?: any): any {
        try {
            ...
            return this.getGeometries(attribute).reduce((pointArray: any, wkt: any) => pointArray.concat(TurfMeta.coordAll(wkx.Geometry.parse(wkt).toGeoJSON())), []);
        }
        catch (err) {
            console.error(err);
            return [];
    ...
    getMapActions() {
        return this.plain.actions.filter((action) => action.id.indexOf('catalog.data.metacard.map.') === 0);
    }
    ...
    getOtherActions() {
        ...
        return this.plain.actions.filter((action) => otherActions.indexOf(action) === -1);
    }
    hasRelevance() {
        return Boolean(this.plain.relevance);
    }
    getRoundedRelevance() {
        return this.plain.relevance.toPrecision((properties as any).relevancePrecision);
    }
    hasErrors() {
