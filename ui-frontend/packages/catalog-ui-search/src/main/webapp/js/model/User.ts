...
(User as any).updateMapLayers = function (layers: any) {
    ...
    const equal = (a: any, b: any) => _.isEqual(_.omit(a, exclude), _.omit(b, exclude));
...
(User as any).MapLayers = Backbone.Collection.extend({
    ...
    defaults() {
        return _.map(_.values(properties.imageryProviders), (layerConfig) => new (User as any).MapLayer(layerConfig, { parse: true }));
    },
...
(User as any).Preferences = Backbone.AssociatedModel.extend({
    ...
    handleResultCount() {
        this.set('resultCount', Math.min((properties as any).resultCount, this.get('resultCount')));
    },
...
(User as any).Response = Backbone.AssociatedModel.extend({
    ...
    getAmPmDisplay() {
        return (this.get('user').get('preferences').get('dateTimeFormat')['timefmt'] ===
