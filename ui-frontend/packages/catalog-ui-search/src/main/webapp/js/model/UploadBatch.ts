...
export default Backbone.AssociatedModel.extend({
  ...
  handleIssuesUpdates() {
    this.set({
      issues: this.get('uploads').reduce((issues: any, upload: any) => {
  ...
  start() {
    if (this.options.dropzone) {
      ...
      ;(wreqr as any).vent.trigger('uploads:add', this)
  ...
  calculatePercentageDone() {
    ...
    const totalBytes = files.reduce((total: any, file: any) => {
      total += file.upload.total
      return total
    }, 0)
    const bytesSent = files.reduce((total: any, file: any) => {
