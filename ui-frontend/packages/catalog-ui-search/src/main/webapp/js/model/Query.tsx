...
import { getConstrainedFinalPageForSourceGroup, getCurrentStartAndEndForSourceGroup, getNextPageForSourceGroup, getPreviousPageForSourceGroup, hasNextPageForSourceGroup, hasPreviousPageForSourceGroup, IndexForSourceGroupType, QueryStartAndEndType, } from './Query.methods';
...
Query.Model = Backbone.AssociatedModel.extend({
    ...
    initialize(attributes: any) {
        ...
        this.listenTo(this, 'change:cql change:filterTree change:sources change:sorts change:spellcheck change:phonetics change:count', () => {
        ...
        this.listenTo(this, 'change:type', () => {
            if (this.get('type') === 'basic') {
                const cleanedUpFilterTree = cql.removeInvalidFilters(this.get('filterTree'));
                this.set('filterTree', downgradeFilterTreeToBasic(cleanedUpFilterTree as any));
            }
        });
    },
    getSelectedSources() {
        ...
        if (selectedSources.includes('remote')) {
            sourceArray = sourceArray
                .concat(_.pluck(Sources.toJSON(), 'id').filter((src: any) => !Sources.getHarvested().includes(src)))
    ...
    buildSearchData() {
        ...
        return _.pick(data, 'sources', 'start', 'count', 'timeout', 'cql', 'sorts', 'id', 'spellcheck', 'phonetics');
    },
    isOutdated() {
    ...
    startSearch(options: any, done: any) {
        ...
        const searchesToRun = [localSearchToRun, ...federatedSearchesToRun].filter((search) => search.srcs.length > 0);
