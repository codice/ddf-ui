...
class MetacardHistory extends React.Component<Props, State> {
    ...
    getSourceId() {
        ...
        const harvestedSourceId = this.model.plain.metacard.properties['ext.harvested-from'];
    ...
    loadData() {
        setTimeout(async () => {
            ...
            history.forEach((historyItem: any, index: any) => {
                ...
                historyItem.niceDate = Common.getMomentDate(moment.unix(historyItem.versioned.seconds).valueOf());
    ...
    revertToSelectedVersion = async () => {
        ...
        const res = await fetch(`./internal/history/revert/${id}/${revertId}/${this.getSourceId()}`);
        if (!res.ok) {
        ...
        setTimeout(() => {
            //let solr flush
            this.model.syncWithPlain();
            if (this.model.plain.metacard.properties['metacard-tags'].indexOf('revision') >= 0) {
                (wreqr as any).vent.trigger('snack', {
    ...
    render() {
        ...
        return (<MetacardHistoryPresentation onClick={this.onClick} revertToSelectedVersion={this.revertToSelectedVersion} history={history} selectedVersion={selectedVersion} loading={loading} canEdit={TypedUserInstance.canWrite(this.model)}/>);
    }
}
