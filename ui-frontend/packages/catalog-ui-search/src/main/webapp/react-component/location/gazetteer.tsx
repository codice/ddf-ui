...
export const getLargestBbox = (polygonCoordinates: any[], isMultiPolygon: boolean) => {
    ...
    const encompassingBoundingBoxHeight = encompassingBoundingBox.maxY - encompassingBoundingBox.minY;
    const encompassingBoundingBoxWidth = encompassingBoundingBox.maxX - encompassingBoundingBox.minX;
...
const Gazetteer = (props: Props) => {
    ...
    const suggesterWithLiteralSupport = async (input: string) => {
        const res = await fetch(`./internal/geofeature/suggestions?q=${encodeURIComponent(input)}`);
    ...
    const geofeatureWithLiteralSupport = async (suggestion: Suggestion) => {
        ...
        const finalArea = getLargestBbox(data.geometry.coordinates, isMultiPolygon(data.geometry.coordinates));
        return {
            type: 'Feature',
            geometry: {
                type: 'Polygon',
    ...
    const suggester = async (input: string) => {
        const res = await (window as any).__global__fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(input)}`);
    ...
    const geofeature = async (suggestion: Suggestion) => {
        ...
        const res = await (window as any).__global__fetch(`https://nominatim.openstreetmap.org/reverse?format=json&osm_type=${type}&osm_id=${id}&polygon_geojson=1`);
        const data = await res.json();
        const boundingBoxLimit = 75;
        ...
        if ((boundingBoxWidth >= boundingBoxLimit ||
            ...
            const finalArea = getLargestBbox(data.geojson.coordinates, isMultiPolygon(data.geojson.coordinates));
    ...
    return (<>
      {onlineGazetteer ? (<Keyword value={props.value} setState={props.setState} suggester={suggester} geofeature={geofeature} placeholder={props.placeholder} loadingMessage={props.loadingMessage} variant={props.variant}/>) : (<Keyword value={props.value} setState={props.setState} suggester={suggesterWithLiteralSupport} geofeature={geofeatureWithLiteralSupport} placeholder={props.placeholder} loadingMessage={props.loadingMessage} variant={props.variant}/>)}
    </>);
};
