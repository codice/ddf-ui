...
const SearchSettings = () => {
    const [queryModel] = React.useState(UserQuery() // we pass this to query settings
    );
    ...
    return (<Root>
      <Tooltip placement="right" title={<Paper elevation={Elevations.overlays} className="p-3">
        ...
        <div>
          ...
          <Grid className={`w-full ${MuiOutlinedInputBorderClasses}`} container alignItems="center" direction="column">
            <Grid item className="w-full">
              <Input fullWidth value={resultCount} margin="dense" onChange={(e) => {
            user.getPreferences().set({
                resultCount: Math.min(parseInt(e.target.value), (properties as any).resultCount),
            });
        }} inputProps={{
            ...
            <Grid item className="w-full px-10">
              <Slider value={resultCount} onChange={(_e, newValue) => {
            user.getPreferences().set({
                resultCount: newValue,
            });
        }} aria-labelledby="input-slider" min={1} max={(properties as any).resultCount} step={10} marks={[
            {
                value: 1,
                label: '1',
            },
