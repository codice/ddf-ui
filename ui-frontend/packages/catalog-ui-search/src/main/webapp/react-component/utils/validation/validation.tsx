...
export const ErrorComponent = (props: any) => {
    ...
    return errorState.error ? (<Invalid className="my-2">
...
export function validateListOfPoints(coordinates: any[], mode: string) {
    ...
    if (!mode.includes('multi') &&
        ...
        message = `Minimum of ${numPoints} points needed for ${isLine ? 'Line' : 'Polygon'}`;
    }
...
function validateUtmUps(key: string, value: any) {
    ...
    const isLatLonValid = !hasPointError([lon, lat]) ||
        northing === undefined ||
        easting === undefined;
...
function getDefaultingErrorMessage(value: string, label: string, defaultValue: number) {
    return `${value.replace(/_/g, '0')} is not an acceptable ${label} value. Defaulting to ${defaultValue}`;
}
